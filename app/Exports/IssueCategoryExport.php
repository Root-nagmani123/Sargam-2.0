...
/**
 * Manage Categories → .xlsx
 *
 * Columns are handed in already resolved by
 * IssueCategoryController::resolveExportColumns(), which is the same array the
 ...
 */
class IssueCategoryExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithEvents,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'MANAGE CATEGORIES');
                ...
                foreach ($this->columns as $key => $col) {
                    if (in_array($key, ['sno', 'sub_categories', 'status'], true)) {
