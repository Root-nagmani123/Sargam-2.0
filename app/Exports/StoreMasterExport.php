...
/**
 * Styled Excel (.xlsx) export for the Store Master listing.
 *
 * A plain CSV can't carry colour/borders/logos, so this builds a formatted
 * workbook whose header block + table styling mirror the branded Print / PDF
 * layout: institution logos, blue title band, blue column header, bordered
 * zebra rows. Kept in step with {@see \App\Http\Controllers\Mess\StoreController::export()}.
 */
class StoreMasterExport implements FromCollection, WithColumnWidths, WithEvents, WithTitle
{
    ...
    private function columnDefinitions(): array
    {
        return [
            ...
            1 => ['key' => 'store_name', 'width' => 42, 'align' => null],
            2 => ['key' => 'store_type', 'width' => 16, 'align' => 'center'],
            3 => ['key' => 'location',   'width' => 30, 'align' => null],
    ...
    private function recordsQuery()
    {
        ...
        // Same order the listing uses (StoreController@index → orderByDesc('id')).
        return $query->orderByDesc('id')->get();
    }

    private function mapRecord($record): array
    {
        $types = Store::storeTypes();
        $type = $record->store_type ?: Store::TYPE_MESS;
        $status = $record->status ?: Store::STATUS_ACTIVE;

        // Name + code on two lines, matching the on-screen "Store Name" cell.
        $name = (string) ($record->store_name ?? '');
