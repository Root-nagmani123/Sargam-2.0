...
/**
 * Branded .xlsx for the Update Meter Details listing.
 *
 * The Print view (admin/estate/update_meter_no_print.blade.php)
 ...
 */
class EstateUpdateMeterNoExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithStyles,
    ...
    WithCustomStartCell
{
    ...
    {
        // Rows arrive already formatted by mapUpdateMeterNoListRow(), so these
        // just pick fields — and normalise its em-dash placeholder to a hyphen.
        // Double quotes: \u{2014} is only an escape sequence in a double-quoted string.
        $show = static fn ($v) => ($v === null || $v === '' || $v === "\u{2014}") ? '-' : (string) $v;

        return [
            'sno' => ['heading' => 'S. No.', 'center' => true, 'width' => 4,
                'value' => fn ($row, int $serial) => $serial],
            'name_id' => ['heading' => 'Name & ID', 'center' => false, 'width' => 14,
                'value' => function ($row) {
            ...
            'employee_type' => ['heading' => 'Employee Type', 'center' => false, 'width' => 10,
                'value' => fn ($row) => $show($row->employee_type ?? null)],
            'unit_type' => ['heading' => 'Unit Type', 'center' => false, 'width' => 8,
                'value' => fn ($row) => $show($row->unit_type ?? null)],
            'unit_sub_type' => ['heading' => 'Unit Sub Type', 'center' => false, 'width' => 8,
                'value' => fn ($row) => $show($row->unit_sub_type ?? null)],
            'building_name' => ['heading' => 'Building Name', 'center' => false, 'width' => 12,
                'value' => fn ($row) => $show($row->building_name ?? null)],
            'house_no' => ['heading' => 'House Number', 'center' => true, 'width' => 8,
                'value' => fn ($row) => $show($row->house_no ?? null)],
            'old_meter1_no' => ['heading' => 'Old Meter 1 No.', 'center' => true, 'width' => 7,
                'value' => fn ($row) => $show($row->old_meter1_no ?? null)],
            'new_meter1_no' => ['heading' => 'New Meter 1 No.', 'center' => true, 'width' => 7,
                'value' => fn ($row) => $show($row->new_meter1_no ?? null)],
            'old_meter2_no' => ['heading' => 'Old Meter 2 No.', 'center' => true, 'width' => 7,
                'value' => fn ($row) => $show($row->old_meter2_no ?? null)],
            'new_meter2_no' => ['heading' => 'New Meter 2 No.', 'center' => true, 'width' => 7,
                'value' => fn ($row) => $show($row->new_meter2_no ?? null)],
            'old_meter1_reading' => ['heading' => 'Old Meter 1 Reading', 'center' => true, 'width' => 8,
                'value' => fn ($row) => $show($row->old_meter1_reading ?? null)],
            'new_meter1_reading' => ['heading' => 'New Meter 1 Reading', 'center' => true, 'width' => 8,
                'value' => fn ($row) => $show($row->new_meter1_reading ?? null)],
            'old_meter2_reading' => ['heading' => 'Old Meter 2 Reading', 'center' => true, 'width' => 8,
                'value' => fn ($row) => $show($row->old_meter2_reading ?? null)],
            'new_meter2_reading' => ['heading' => 'New Meter 2 Reading', 'center' => true, 'width' => 8,
                'value' => fn ($row) => $show($row->new_meter2_reading ?? null)],
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'UPDATE METER DETAILS');
