...
/**
 * Branded .xlsx for the Define House grid.
 *
 * The Print view (admin/estate/define_house_print.blade.php) renders from the
 ...
 */
class EstateDefineHouseExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithStyles,
    ...
    WithCustomStartCell
{
    ...
    /**
     * The label the grid shows for a house.
     *
     * Under Renovation is a property of the house itself
     * (vacant_renovation_status = 0) and outranks everything. Occupied has two
     * sources: the allotment flag used_home_status, and an explicit
     * vacant_renovation_status = 2 chosen on the Define House form — without the
     * second the form's "Occupied" option saved but never showed. Kept here so
     * the grid, the sheet and the printout can never disagree.
     */
    public static function statusLabel($row): string
    {
        $renovation = (int) ($row->vacant_renovation_status ?? 1);
        if ($renovation === 0) {
            return 'Under Renovation';
        }

        return ($renovation === 2 || (int) ($row->used_home_status ?? 0) === 1) ? 'Occupied' : 'Vacant';
    }
     ...
     * The table's Action column is deliberately absent — it is a set of controls,
     * not data.
     */
    public static function columnDefs(): array
    {
        return [
            ...
            'unit_type' => ['heading' => 'Unit Type', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->unit_type ?? null)],
            'building_name' => ['heading' => 'Building Name', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->building_name ?? null)],
            'unit_sub_type' => ['heading' => 'Unit Sub Type', 'center' => false, 'money' => false,
            ...
            'meter_one' => ['heading' => 'Meter Number 1', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->meter_one ?? null)],
            'water_charge' => ['heading' => 'Water Charge', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->water_charge ?? 0)],
            'electric_charge' => ['heading' => 'Electric Charge', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->electric_charge ?? 0)],
            'licence_fee' => ['heading' => 'Licence Fee', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->licence_fee ?? 0)],
            'status' => ['heading' => 'Status', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::statusLabel($row)],
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'DEFINE HOUSE');
