...
/**
 * Peer Evaluation -> Manage Evaluation Columns -> .xlsx
 *
 * Columns arrive already resolved by PeerEvaluationColumnController::resolveExportColumns(),
 * the same array the CSV, the PDF and the print sheet use - so hiding a column in
 * the grid's Columns modal drops it from all four formats at once.
 *
 ...
 */
class PeerEvaluationColumnExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithEvents,
    ...
    WithCustomStartCell
{
    ...
    /** Column keys rendered centred in the grid, kept centred here. */
    private const CENTRED = ['sno', 'max_marks', 'buffer_marks', 'has_remarks', 'created_date', 'status'];
    ...
    public function title(): string
    {
        ...
        $name = $this->reportTitle ?: 'Evaluation Columns';
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', mb_strtoupper($this->reportTitle ?: 'Evaluation Columns'));
                ...
                foreach ($this->columns as $key => $col) {
                    // Every crit_* column is a score, so centre those as well as
                    // the fixed list - the criteria set is dynamic.
                    if (in_array($key, self::CENTRED, true) || str_starts_with((string) $key, 'crit_')) {
