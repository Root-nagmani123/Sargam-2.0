...
/**
 * Branded .xlsx for the Change Request For House grid.
 *
 * The Print view (admin/estate/request_for_house_print.blade.php) renders from the
 ...
 */
class EstateChangeRequestHouseExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithStyles,
    ...
    WithCustomStartCell
{
    ...
    public static function columnDefs(): array
    {
        return [
            ...
            'request_id' => ['heading' => 'Request ID', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->request_id ?? null)],
            'request_date' => ['heading' => 'Request Date', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->request_date ?? null)],
            ...
            'doj_academy' => ['heading' => 'Date of Joining in Academy', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->doj_academy ?? null)],
            'status' => ['heading' => 'Status of Request', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->status ?? null)],
            'alloted_house' => ['heading' => 'Alloted House', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->alloted_house ?? null)],
            'eligibility_type' => ['heading' => 'Eligibility Type', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->eligibility_type ?? null)],
            'possession_from' => ['heading' => 'Possession From', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->possession_from ?? null)],
            'possession_to' => ['heading' => 'Possession To', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->possession_to ?? null)],
        ];
    }
    ...
    {
        return 'Change Request For House';
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'CHANGE REQUEST FOR HOUSE');
