...
/**
 * Branded .xlsx for the Generate Estate Bill for Other grid (contract staff).
 *
 * The Print view (admin/estate/generate_estate_bill_for_other_print.blade.php)
 * renders from the SAME columnDefs(), so the printout and the .xlsx always carry
 * the same header row and the same column set — including whatever the user hid
 * in the on-screen "Columns" modal, which arrives as ?cols=.
 *
 * Money columns are emitted as real numbers (flagged `money`) and given a rupee
 * number format, so the sheet stays summable; the print view formats the same
 * raw value for paper.
 */
class EstateGenerateBillForOtherExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithStyles,
    ...
    WithCustomStartCell
{
     ...
     * The table's leading checkbox column is deliberately absent — it is a
     * selection control, not data.
     */
    public static function columnDefs(): array
    {
        return [
            ...
            'name_id' => ['heading' => 'Name & ID', 'center' => false, 'money' => false,
                'value' => function ($row) {
                    $name = self::text($row->name ?? null);
                    $id = trim((string) ($row->request_no ?? ''));
                    if ($name === '-') {
                        return $id !== '' ? $id : '-';
                    }
            ...
            'section' => ['heading' => 'Section Name', 'center' => false, 'money' => false,
                'value' => fn ($row) => self::text($row->section ?? null)],
            'house_no' => ['heading' => 'House Number', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->house_no ?? null)],
            'from_date' => ['heading' => 'From Date', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->from_date ?? null)],
            'to_date' => ['heading' => 'To Date', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->to_date ?? null)],
            'meter_no' => ['heading' => 'Meter No.', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->meter_no ?? null)],
            'prev_reading' => ['heading' => 'Previous Meter Reading', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->prev_reading ?? null)],
            'curr_reading' => ['heading' => 'Current Meter Reading', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->curr_reading ?? null)],
            'unit_consumed' => ['heading' => 'Unit Consumed', 'center' => true, 'money' => false,
                'value' => fn ($row) => self::text($row->unit_consumed ?? null)],
            'total_charge' => ['heading' => 'Total Charge', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->total_charge ?? 0)],
            'licence_fee' => ['heading' => 'Licence Fee', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->licence_fee ?? 0)],
            'water_charges' => ['heading' => 'Water Fee', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->water_charges ?? 0)],
            'grand_total' => ['heading' => 'Grand Total', 'center' => false, 'money' => true,
                'value' => fn ($row) => (float) ($row->grand_total ?? 0)],
        ];
    }
     ...
     *
     * Two-meter rows arrive newline-joined from the grid payload; the newline is
     * meaningful (one line per meter) so it is kept, not flattened.
     */
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if ($lastRow >= $firstDataRow) {
            $sheet->getStyle("A{$firstDataRow}:{$lastCol}{$lastRow}")->applyFromArray([
                ...
                // Two-meter rows carry a newline, so the cell has to wrap or the
                // second meter is invisible in the sheet.
            ...
            foreach ($this->cols as $key) {
                ...
                // Money stays numeric — formatted, not stringified — so the sheet
                // can still be summed.
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'GENERATE ESTATE BILL FOR OTHER');
