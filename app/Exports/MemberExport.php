...
class MemberExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    ...
    {
        // $roles = '';
        // if($row->assignedRoles()) {
        //     $roles = $row->assignedRoles()->implode(', ');
        // }

        return [
            ++$this->index,
            $row->title ? EmployeeMaster::title[$row->title] : '',
            $row->first_name,
            $row->middle_name,
            $row->last_name,
            $row->father_name,
            $row->dob,
            EmployeeMaster::gender[$row->gender] ?? '',
            EmployeeMaster::maritalStatus[$row->marital_status] ?? '',
            $row->nationality,
            $row->height,
            $row->email,
            $row->mobile,
            ...
            City::find($row->city)->city_name ?? '',
            State::find($row->state_master_pk)->state_name ?? '',
            Country::find($row->country_master_pk)->country_name ?? '',
            $row->zipcode,
            $row->emp_id,
