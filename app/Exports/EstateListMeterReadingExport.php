...
/**
 * Branded .xlsx for the List Meter Reading grid.
 *
 * The Print view (admin/estate/list_meter_reading_print.blade.php)
 ...
 */
class EstateListMeterReadingExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithStyles,
    ...
    WithCustomStartCell
{
    ...
    public static function columnDefs(): array
    {
        return [
            ...
            'name_id' => ['heading' => 'Name & ID', 'center' => false, 'width' => 18,
                'value' => function ($row) {
            ...
            'designation' => ['heading' => 'Designation', 'center' => false, 'width' => 14,
                'value' => fn ($row) => self::text($row->designation ?? null)],
            'section' => ['heading' => 'Section', 'center' => false, 'width' => 14,
                'value' => fn ($row) => self::text($row->section ?? null)],
            'unit_type' => ['heading' => 'Unit Type', 'center' => false, 'width' => 10,
                'value' => fn ($row) => self::text($row->unit_type ?? null)],
            'unit_sub_type' => ['heading' => 'Unit Sub Type', 'center' => false, 'width' => 10,
                'value' => fn ($row) => self::text($row->unit_sub_type ?? null)],
            'building_name' => ['heading' => 'Building Name', 'center' => false, 'width' => 14,
                'value' => fn ($row) => self::text($row->building_name ?? null)],
            'house_no' => ['heading' => 'House Number', 'center' => true, 'width' => 9,
                'value' => fn ($row) => self::text($row->house_no ?? null)],
            'meter1_reading' => ['heading' => 'Meter 1 Reading', 'center' => true, 'width' => 9,
                'value' => fn ($row) => self::text($row->meter1_reading ?? null)],
            'meter2_reading' => ['heading' => 'Meter 2 Reading', 'center' => true, 'width' => 9,
                'value' => fn ($row) => self::text($row->meter2_reading ?? null)],
        ];
    }

    /** A stored value, or the export's dash for the grid's "N/A" blanks. */
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'LIST METER READING');
