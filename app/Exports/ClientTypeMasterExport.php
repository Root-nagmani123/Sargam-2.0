...
/**
 * Styled Excel (.xlsx) export for the Client Master listing.
 *
 * Mirrors {@see StoreMasterExport}. Kept in step with
 * {@see \App\Http\Controllers\Mess\ClientTypeController::export()}.
 */
class ClientTypeMasterExport implements FromCollection, WithColumnWidths, WithEvents, WithTitle
{
    protected ?string $search;

    /**
     * On-screen data-column indexes (0..3) left visible via the Column Visibility
     * modal, or null when every column shows. S.No (0) is always kept; Action (4)
     * is never exported.
     *
    ...
    private function columnDefinitions(): array
    {
        return [
            ...
            1 => ['key' => 'client_type', 'width' => 22, 'align' => null],
            2 => ['key' => 'client_name', 'width' => 40, 'align' => null],
            3 => ['key' => 'status',      'width' => 14, 'align' => 'center'],
        ];
    }
    ...
    {
        $types = ClientType::clientTypes();
        $type = (string) ($record->client_type ?? '');

        return [
            'client_type' => $types[$type] ?? $type,
            'client_name' => (string) ($record->client_name ?? ''),
            'status'      => ucfirst($record->status ?: ClientType::STATUS_ACTIVE),
        ];
    }
