...
/**
 * Branded .xlsx for the Estate Possession for Others listing.
 *
 * The Print view (admin/estate/estate_possession_for_others_print.blade.php)
 ...
 */
class EstatePossessionOtherExport implements
    FromArray,
    WithHeadings,
    ShouldAutoSize,
    WithStyles,
    ...
    WithCustomStartCell
{
    ...
    public static function columnDefs(): array
    {
        return [
            ...
            'name' => ['heading' => 'Employee Name', 'center' => false, 'width' => 16,
                'value' => fn ($row) => trim((string) ($row->emp_name ?? '')) ?: '-'],
            'section_name' => ['heading' => 'Section Name', 'center' => false, 'width' => 13,
                'value' => fn ($row) => EstatePossessionOtherDataTable::sectionLabel($row) ?: '-'],
            'estate_name' => ['heading' => 'Estate Name', 'center' => false, 'width' => 11,
                'value' => fn ($row) => $row->campus_name ?: '-'],
            'building_name' => ['heading' => 'Building Name', 'center' => false, 'width' => 11,
                'value' => fn ($row) => $row->block_name ?: '-'],
            'unit_type' => ['heading' => 'Unit Type', 'center' => false, 'width' => 8,
                'value' => fn ($row) => $row->unit_type_name ?: '-'],
            'unit_sub_type' => ['heading' => 'Unit Sub Type', 'center' => false, 'width' => 8,
                'value' => fn ($row) => $row->unit_sub_type_name ?: '-'],
            'house_no' => ['heading' => 'House Number', 'center' => true, 'width' => 8,
                'value' => fn ($row) => $row->house_no ?: ($row->house_no_display ?: '-')],
            ...
            'possession_date' => ['heading' => 'Possession Date', 'center' => true, 'width' => 9,
                'value' => fn ($row) => $row->possession_date_oth
                    ? \Carbon\Carbon::parse($row->possession_date_oth)->format('d-m-Y')
                    : '-'],
            'electric_meter_reading' => ['heading' => 'Last Electric Bill Reading', 'center' => true, 'width' => 10,
                'value' => fn ($row) => EstatePossessionOtherDataTable::meterReadingLabel($row)],
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A2', 'ESTATE POSSESSION FOR OTHERS');
