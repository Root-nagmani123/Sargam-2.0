...
class ProcessMessBillsEmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($isDataTableRequest) {
            ...
            $this->warmModalBillSummaryCacheFromCombinedBills(
                $combinedBills,
                $dateFrom,
                $dateTo,
                $clientTypes,
    ...
    private function processMessBillsDatatableResponse(
        ...
        $skipLifetimeDue = $request->boolean('skip_lifetime_due');
        ...
        $needsFullLifetimeDueForSort = isset($sortMap[$orderColumn]) && $sortMap[$orderColumn] === 'total_due_amount';
        if ($needsFullLifetimeDueForSort && ! $skipLifetimeDue && ! $lifetimeDueOnly) {
            $filteredBills = $this->enrichProcessIndexPageLifetimeDue($filteredBills, $effectiveDateToYmd);
        }
        if ($orderColumn === 0) {
        ...
        if ($lifetimeDueOnly) {
            $pageRows = $filteredBills->slice($start, $length)->values();
            $enriched = $this->enrichProcessIndexPageLifetimeDue($pageRows, $effectiveDateToYmd);

            return response()->json([
                'dues' => $enriched->map(function ($cb) {
                    return [
        ...
        if (! $skipLifetimeDue && ! $needsFullLifetimeDueForSort) {
            $rows = $this->enrichProcessIndexPageLifetimeDue($rows, $effectiveDateToYmd);
        }
    ...
    private function getCombinedBillsForProcessIndexCached(
        ...
        $combinedBills = $this->rememberProcessMessBillsCombined(
            ...
            function () use ($dateFrom, $dateTo, $dateRangeQuery, $kitchenIssueQuery) {
                return $this->queryAndGroupBillsForProcessIndexLight(
                    $dateFrom,
                    $dateTo,
                    $dateRangeQuery,
                    $kitchenIssueQuery,
     ...
     */
    private function modalBillsSummaryCacheKey(
        string $dateFrom,
        string $dateTo,
        array $clientTypes,
        array $clientTypePks,
        ...
        return 'process_mess_bills_modal_summary_v4:'
            . $this->processMessBillsCombinedCacheVersion()
            . ':'
            . md5(json_encode([
                'from' => $dateFrom,
    ...
    /**
     * Cached lightweight modal summaries (due > 0). Notifications and lifetime due are resolved per page only.
     *
     * @param  array<int, string>  $clientTypes
     * @param  array<int, string|int>  $clientTypePks
     * @param  array<int, string>  $buyerNames
     ...
     */
    private function getModalBillSummariesCached(
        string $dateFrom,
        string $dateTo,
        $dateRangeQuery,
        $kitchenIssueQuery,
        ...
        $rows = $this->rememberProcessMessBillsCombined(
            $cacheKey,
            function () use ($dateFrom, $dateTo, $dateRangeQuery, $kitchenIssueQuery, $clientTypes, $clientTypePks, $buyerNames) {
                $fromCombined = $this->modalSummariesFromProcessIndexCombinedCacheIfAvailable(
                    $dateFrom,
                    $dateTo,
                    $clientTypes,
                    $clientTypePks,
                ...
                return $this->buildModalBillSummariesUncached(
                    $dateFrom,
                    $dateTo,
                    $dateRangeQuery,
                    $kitchenIssueQuery
    ...
    /**
     * Build modal buyer summaries once (no notifications, no line-item keys, no lifetime FIFO).
     *
     * @return list<array<string, mixed>>
     */
    private function buildModalBillSummariesUncached(
        string $dateFrom,
        string $dateTo,
        $dateRangeQuery,
        $kitchenIssueQuery
        ...
        return $groupedRows
            ...
            ->map(fn ($cb) => $this->buildModalBillSummaryFromGroup($cb, $dateFrom, $dateTo))
            ->values()
            ->all();
    }
     ...
     */
    private function modalSummariesFromProcessIndexCombinedCacheIfAvailable(
        string $dateFrom,
        string $dateTo,
        array $clientTypes,
        array $clientTypePks,
        ...
        if ($clientTypes !== [] || $clientTypePks !== [] || $buyerNames !== []) {
            return null;
        }

        $combinedCacheKey = $this->combinedBillsCacheKey($dateFrom, $dateTo, [
            'client_types' => [],
            'client_type_pks' => [],
            'buyer_names' => [],
            'status_filter' => null,
            'invoice_sent_filter' => null,
            'context' => 'index',
        ]);

        try {
            $cached = $this->processMessBillsCacheRepository()->get($combinedCacheKey);
        } catch (\Throwable $e) {
            return null;
        }
        ...
        $combinedBills = $cached instanceof Collection ? $cached : collect(is_array($cached) ? $cached : []);

        return $this->modalSummariesFromCombinedBillRows($combinedBills, $dateFrom, $dateTo);
    }

    /**
     * @return list<array<string, mixed>>
     */
    private function modalSummariesFromCombinedBillRows(
        Collection $combinedBills,
        string $dateFrom,
        string $dateTo
    ): array {
    ...
    /**
     * After the main grid loads, seed the modal summary cache without a second union query.
     *
     * @param  array<int, string>  $clientTypes
     * @param  array<int, string|int>  $clientTypePks
     * @param  array<int, string>  $buyerNames
     */
    private function warmModalBillSummaryCacheFromCombinedBills(
        Collection $combinedBills,
        string $dateFrom,
        string $dateTo,
        array $clientTypes,
     ...
     */
    private function buildModalBillSummaryFromGroup(object $cb, string $dateFrom, string $dateTo): array
    {
        ...
        $clientId = 0;
        foreach ($cb->bills ?? [] as $bill) {
            $billClientId = (int) ($bill->client_id ?? 0);
            if ($billClientId > 0) {
        ...
        return [
            ...
            'client_id' => $clientId,
            'receiver_user_id' => 0,
            'bill_stub_keys' => array_values(array_filter(
                (array) ($cb->bill_stub_keys ?? []),
    ...
    /**
     * Resolve notifications + line-item keys for the current modal page only.
     *
     * @param  \Illuminate\Support\Collection<int, array<string, mixed>>  $pageSummaries
     * @return \Illuminate\Support\Collection<int, array<string, mixed>>
     */
    private function buildModalBillRowsForPageSummaries(
        Collection $pageSummaries,
        string $dateFrom,
        string $dateTo
    ): Collection {
        ...
        $stubKeys = [];
        $receiverIds = [];
        $pageSummaries = $pageSummaries->map(function (array $summary) use (&$stubKeys, &$receiverIds) {
            $summary['receiver_user_id'] = $this->resolveReceiverUserIdForModalSummary($summary);
            foreach ($summary['bill_stub_keys'] ?? [] as $stubKey) {
                $stubKeys[$stubKey] = true;
            }
            $receiverId = (int) ($summary['receiver_user_id'] ?? 0);
            if ($receiverId > 0) {
        ...
        $stubLineKeysMap = $this->batchLineItemKeysByStubKeyStrings(array_keys($stubKeys));
        $this->preloadMessCombinedNotificationsForReceivers(array_keys($receiverIds));

        return $pageSummaries->map(function (array $summary) use ($dateFrom, $dateTo, $stubLineKeysMap) {
            $lineItemKeys = [];
            foreach ($summary['bill_stub_keys'] ?? [] as $stubKey) {
                foreach ($stubLineKeysMap[$stubKey] ?? [] as $lineKey) {
                    $lineItemKeys[] = $lineKey;
                }
            }
            $lineItemKeys = array_values(array_unique($lineItemKeys));

            $cb = (object) [
                'combined_id' => $summary['id'],
                'combined_invoice_no' => $summary['invoice_no'],
                'buyer_name' => $summary['buyer_name'],
                'client_type_slug' => $summary['client_type_slug'],
                'payment_type' => $summary['payment_type'],
                'total' => $summary['total'],
                'paid' => $summary['paid'],
                'due' => $summary['due'],
                'receiver_user_id' => $summary['receiver_user_id'],
                'line_item_keys' => $lineItemKeys,
            ];

            return $this->buildModalBillCachedRow($cb, $dateFrom, $dateTo, []);
        });
    }

    /**
     * Resolve notification receiver for one modal row (deferred from summary cache build).
     *
     * @param  array<string, mixed>  $summary
     */
    private function resolveReceiverUserIdForModalSummary(array $summary): int
    {
        $buyerName = trim((string) ($summary['buyer_name'] ?? ''));
        $slug = (string) ($summary['client_type_slug'] ?? 'employee');
        $clientId = (int) ($summary['client_id'] ?? 0);
        ...
        if (in_array($slug, ['ot', 'course'], true) && $clientId > 0) {
            return $clientId;
        }

        if ($slug === 'employee' || $slug === 'other') {
            return (int) ($this->resolveReceiverUserIdByClientName($buyerName) ?? 0);
        }

        if (in_array($slug, ['ot', 'course'], true)) {
            return (int) ($this->resolveReceiverUserIdByStudentName($buyerName) ?? 0);
        }
    ...
    public function myBillsIndex(Request $request)
    {
        ...
        $combinedBills = $this->filterCombinedBillsByInvoiceSent($combinedBills, 'sent', $dateFrom, $dateTo);
        $combinedBills = $combinedBills
            ->map(fn ($cb) => $this->applyMyMessBillInvoicedOnlyToCombinedBill($cb, $dateFrom, $dateTo))
            ->filter()
            ->values();
    ...
    private function queryAndGroupBillsForProcessIndexLight(
        ...
        $stubLineKeysMap = $skipLineItemKeys
            ? []
    ...
    private function batchLineItemKeysByProcessIndexStubKey(Collection $stubs): array
    {
        ...
        return $this->batchLineItemKeysByDrAndKiIds(array_keys($drIds), array_keys($kiPks));
    }

    /**
     * @param  array<int, string>  $stubKeys  e.g. dr-12, ki-34
     * @return array<string, array<int, string>>
     */
    private function batchLineItemKeysByStubKeyStrings(array $stubKeys): array
    {
        $drIds = [];
        $kiPks = [];
        foreach ($stubKeys as $stubKey) {
            if (preg_match('/^dr-(\d+)$/i', (string) $stubKey, $matches)) {
                $drIds[(int) $matches[1]] = true;
            } elseif (preg_match('/^ki-(\d+)$/i', (string) $stubKey, $matches)) {
                $kiPks[(int) $matches[1]] = true;
            }
        }
    ...
    /**
     * @param  array<int, int>  $drIdList
     * @param  array<int, int>  $kiPkList
     * @return array<string, array<int, string>>
     */
    private function batchLineItemKeysByDrAndKiIds(array $drIdList, array $kiPkList): array
    {
        $map = [];
    ...
    /**
     * Lifetime FIFO due for the current process-index page only (not all buyers).
     *
     * @param  \Illuminate\Support\Collection<int, object>  $pageRows
     * @return \Illuminate\Support\Collection<int, object>
     */
    private function enrichProcessIndexPageLifetimeDue(Collection $pageRows, ?string $dateToYmd): Collection
    {
        if (! $dateToYmd || $pageRows->isEmpty()) {
            return $pageRows;
        }
        ...
        return $pageRows->map(function ($cb) use ($dateToYmd) {
            $buyerName = trim((string) ($cb->buyer_name ?? ''));
            if ($buyerName === '' || $buyerName === '—') {
                return $cb;
            }
    ...
    {
        return $this->enrichProcessIndexPageLifetimeDue($combinedBills, $dateToYmd);
    }
    ...
    private function groupProcessIndexVouchersByBuyer(
        ...
        return $vouchers->groupBy(fn ($bill) => $deferReceiverUserId
            ...
            $buyerName = $deferReceiverUserId
                ...
                : $this->resolveMessBillBuyerDisplayName($first, $group);
    ...
    /**
     * Fast buyer group key for modal summary cache (uses stub client_id only, no name lookups).
     */
    private function messBillBuyerGroupKeyFast($bill): string
    {
        $slug = $this->getBillClientTypeSlug($bill);
        $clientId = (int) ($bill->client_id ?? 0);
        if ($clientId > 0 && in_array($slug, self::ALLOWED_CLIENT_SLUGS, true)) {
            return 'cid:' . $clientId . '|' . $slug;
        }
     ...
     */
    private function resolveMessBillBuyerDisplayNameFast($bill, $group = null): string
    {
        ...
        return trim((string) ($bill->client_name ?? ''));
    }
    ...
    private function getMessCombinedNotifiedLineItemKeys(
        ...
        $combinedId = $this->normalizeMessCombinedBillId($combinedId);
        $notified = [];
        ...
        foreach ($this->messCombinedNotificationsForReceiver($receiverUserId) as $n) {
            ...
            if ($parsed === null || ! $this->messCombinedBillIdsMatch((string) ($parsed['i'] ?? ''), $combinedId)) {
                continue;
            }
            $nf = (string) ($parsed['f'] ?? '');
            $nt = (string) ($parsed['t'] ?? '');
    ...
    /**
     * My Mess Bills (user only): keep invoiced line items and recalculate list totals.
     */
    private function applyMyMessBillInvoicedOnlyToCombinedBill(object $cb, string $dateFromYmd, string $dateToYmd): ?object
    {
        $bills = $cb->bills->all();
        $receiverId = (int) ($this->resolveReceiverUserIdFromAnyBill($bills) ?? 0);
        if ($receiverId <= 0) {
            return null;
        }
        ...
        $filteredBills = $this->filterBillsToMessLineItemKeys($bills, $notifiedKeys);
        if ($filteredBills === []) {
            return null;
        }

        $buyerName = trim((string) ($filteredBills[0]->client_name ?? ($filteredBills[0]->clientTypeCategory->client_name ?? '')));
        $clientTypeSlug = $filteredBills[0] instanceof SellingVoucherDateRangeReport
            ? (string) ($filteredBills[0]->client_type_slug ?? 'employee')
            : $this->getBillClientTypeSlug($filteredBills[0]);
        $financials = $this->computeMyMessBillFinancialsForNotifiedKeys(
            $buyerName,
            $clientTypeSlug,
            $notifiedKeys,
            $dateFromYmd,
        ...
        if (($financials['total'] ?? 0) <= 0) {
            return null;
        }

        $cb->total = $financials['total'];
        $cb->paid = $financials['paid'];
        $cb->due = $financials['due'];
        $cb->status = $this->isBillFullyPaid($financials['paid'], $financials['total'])
            ? 2
            : ($financials['paid'] > 0 ? 1 : 0);
        $cb->invoice_date_range = $this->resolveMyMessBillInvoiceDateRangeFromBills($filteredBills);

        return $cb;
    }

    /**
     * My Mess Bills details/print (user only): restrict combined bill rows to invoiced line items.
     *
     * @param  array<int, SellingVoucherDateRangeReport|KitchenIssueMaster>  $bills
     * @return array{0: array, 1: array{total: float, paid: float, due: float}, 2: string, 3: string}
     */
    private function restrictUserCombinedBillsToInvoicedOnly(
        string $combinedId,
        array $bills,
        string $dateFromYmd,
        string $dateToYmd
        ...
        $receiverId = (int) ($this->resolveReceiverUserIdFromAnyBill($bills) ?? 0);
        if ($receiverId <= 0) {
            return [[], ['total' => 0.0, 'paid' => 0.0, 'due' => 0.0], $dateFromYmd, $dateToYmd];
        }
        ...
        [$fromYmd, $toYmd] = $this->resolveMyMessBillInvoiceDateRangeYmdBounds($filteredBills);
     ...
     * @return array<int, SellingVoucherDateRangeReport|KitchenIssueMaster>
     */
    private function filterBillsToMessLineItemKeys(array $bills, array $allowedKeys): array
    {
        ...
        foreach ($bills as $bill) {
            if ($bill instanceof SellingVoucherDateRangeReport) {
                $items = collect($bill->items ?? [])->filter(function ($item) use ($allowed) {
                    $id = (int) ($item->id ?? 0);

                    return $id > 0 && isset($allowed['dr-' . $id]);
                })->values();
                if ($items->isEmpty()) {
                    continue;
                }
            ...
            if ($bill instanceof KitchenIssueMaster) {
                $items = $bill->relationLoaded('items') ? $bill->items : collect($bill->items ?? []);
                if ($items->isNotEmpty()) {
                    $items = $items->filter(function ($item) use ($allowed) {
                        $pk = (int) ($item->pk ?? 0);

                        return $pk > 0 && isset($allowed['ki-' . $pk]);
                    })->values();
                    if ($items->isEmpty()) {
                        continue;
                    }
                ...
                if ($masterPk > 0 && isset($allowed['ki-bill-' . $masterPk])) {
                    $filtered[] = $bill;
                }
            }
        }
    ...
    /**
     * @param  array<int, string>  $notifiedKeys
     * @return array{total: float, paid: float, due: float}
     */
    private function computeMyMessBillFinancialsForNotifiedKeys(
        string $buyerName,
        ...
        $notifiedSet = array_fill_keys($notifiedKeys, true);
        $lineItems = $this->buildMyMessBillFifoAllocatedLines($buyerName, $clientTypeSlug, $dateToYmd);

        $total = 0.0;
        $paid = 0.0;
        foreach ($lineItems as $line) {
            $lineKey = (string) ($line['line_key'] ?? '');
            if ($lineKey === '' || ! isset($notifiedSet[$lineKey])) {
                continue;
            }
    ...
    /**
     * @return list<array{issue_date_ymd: string, amount: float, allocated_paid: float, bill_key: string, line_key: string}>
     */
    private function buildMyMessBillFifoAllocatedLines(
        string $buyerName,
        string $clientTypeSlug,
        ?string $dateToYmd
    ): array {
        ...
        foreach ($allocationBills as $bill) {
            ...
            foreach ($this->extractKeyedLineItemsFromBill($bill) as $line) {
    ...
    /**
     * @return list<array{issue_date_ymd: string, amount: float, line_key: string}>
     */
    private function extractKeyedLineItemsFromBill($bill): array
    {
        $lines = [];
        if ($bill instanceof SellingVoucherDateRangeReport) {
            foreach ($bill->items ?? [] as $item) {
                ...
                $lines[] = ['issue_date_ymd' => $ymd, 'amount' => $amount, 'line_key' => 'dr-' . $id];
            }
        ...
        if ($bill instanceof KitchenIssueMaster) {
            ...
            foreach ($bill->items ?? [] as $item) {
                ...
                $lines[] = ['issue_date_ymd' => $ymd, 'amount' => $amount, 'line_key' => 'ki-' . $pk];
            }
            if ($lines === [] && $fallbackYmd !== null) {
                ...
                if ($net > 0 && $masterPk > 0) {
                    $lines[] = [
                        'issue_date_ymd' => $fallbackYmd,
                        'amount' => $net,
                        'line_key' => 'ki-bill-' . $masterPk,
                    ];
                }
            }
        }
     ...
     * @param  array<int, SellingVoucherDateRangeReport|KitchenIssueMaster>  $bills
     */
    private function resolveMyMessBillInvoiceDateRangeFromBills(array $bills): string
    {
        [$fromYmd, $toYmd] = $this->resolveMyMessBillInvoiceDateRangeYmdBounds($bills);

        return $fromYmd && $toYmd
            ? (Carbon::parse($fromYmd)->format('d-m-Y') . ($fromYmd !== $toYmd ? ' to ' . Carbon::parse($toYmd)->format('d-m-Y') : ''))
            : '—';
    }
     ...
     */
    private function resolveMyMessBillInvoiceDateRangeYmdBounds(array $bills): array
    {
        $dateStrings = collect();
        foreach ($bills as $b) {
            if ($b instanceof SellingVoucherDateRangeReport && $b->relationLoaded('items') && $b->items->isNotEmpty()) {
        ...
        return [$dateStrings->first(), $dateStrings->last()];
    }
    ...
    /**
     * Canonical combined bill id (route/notification may use spaces or %20 in the name segment).
     */
    private function normalizeMessCombinedBillId(string $combinedId): string
    {
        $parsed = $this->parseProcessMessCombinedBillId($combinedId);
        if ($parsed === null) {
            return $combinedId;
        }

        return 'combined-' . rawurlencode($parsed['name']) . '-' . $parsed['slug'];
    }

    private function messCombinedBillIdsMatch(string $left, string $right): bool
    {
        return $this->normalizeMessCombinedBillId($left) === $this->normalizeMessCombinedBillId($right);
    }
    ...
    public function printReceipt(Request $request, $id)
    {
        ...
        if (is_string($id) && strpos($id, 'combined-') === 0) {
            ...
            $isUserMyMessBillView = $this->isMyMessBillInvoicedOnlyRequest($request);
            if ($isUserMyMessBillView) {
                [$bills, $financials, $invoicedDateFromYmd, $invoicedDateToYmd] = $this->restrictUserCombinedBillsToInvoicedOnly(
                    (string) $id,
                    $bills,
                    $filterDateFromYmd,
                    $filterDateToYmd
                ...
                if ($bills === []) {
                    abort(404, 'No invoiced bill items found for this period.');
                }
            }
            $items = [];
            $storeNames = [];
            ...
            if ($isUserMyMessBillView) {
                ...
                $totalAmount = $financials['total'];
                $paidAmount = $financials['paid'];
                $invoicedDateFromYmd = $filterDateFromYmd;
                $invoicedDateToYmd = $filterDateToYmd;
            }
            ...
            if ($paymentOnly) {
                ...
                $bill = (object) [
                    ...
                    'client_type_slug' => $clientTypeSlug,
                    'resolved_store_name' => '—',
                    'date_from' => Carbon::parse($invoicedDateFromYmd),
                    'date_to' => Carbon::parse($invoicedDateToYmd),
                    'issue_date' => null,
                ...
                return view('admin.mess.process-mess-bills-employee.print-receipt', [
                    ...
                    'totalDueAmount' => $isUserMyMessBillView
                        ? $financials['due']
                        : $this->computeCombinedBillFinancials($buyerName, $clientTypeSlug, null, $filterDateToYmd)['due'],
    ...
    public function paymentDetails(Request $request, $id)
    {
        if (is_string($id) && strpos($id, 'combined-') === 0) {
            ...
            if ($isUserMyMessBillView) {
                ...
                if ($bills === []) {
                    return response()->json(['error' => 'No invoiced bill items found for this period.'], 404);
                }
                $dateFromStr = Carbon::parse($invoicedDateFromYmd)->format('d-m-Y');
                $dateToStr = Carbon::parse($invoicedDateToYmd)->format('d-m-Y');
                $totalAmount = $financials['total'];
                $paidAmount = $financials['paid'];
            } else {
                $dateFromStr = Carbon::parse($filterDateFromYmd)->format('d-m-Y');
                $dateToStr = Carbon::parse($filterDateToYmd)->format('d-m-Y');
                $financials = $this->computeCombinedBillFinancials(
                    trim((string) ($bills[0]->client_name ?? ($bills[0]->clientTypeCategory->client_name ?? '—'))),
                    $bills[0] instanceof SellingVoucherDateRangeReport
                        ? (string) ($bills[0]->client_type_slug ?? 'employee')
                        : $this->getBillClientTypeSlug($bills[0]),
                    $filterDateFromYmd,
                    $filterDateToYmd
    ...
    public function modalData(Request $request)
    {
        ...
        $groupedRows = $this->getModalBillSummariesCached(
            $dateFrom,
            $dateTo,
            $dateRangeQuery,
            $kitchenIssueQuery,
        ...
        $pageSummaries = $groupedRows->slice($offset, $perPage)->values();

        if ($lifetimeDueOnly) {
            $minimalRows = $pageSummaries->map(function (array $summary) {
                return [
                    'id' => $summary['id'],
                    'buyer_name' => $summary['buyer_name'],
                    'client_type_slug' => $summary['client_type_slug'],
                    'total_due_amount' => (float) ($summary['due'] ?? 0),
                ];
            });
            $enriched = $this->enrichModalPageRowsLifetimeDue($minimalRows, $dateTo);

            return response()->json([
                'dues' => $enriched->map(function (array $row) {
                    return [
                        'id' => $row['id'],
                        'total_due_amount' => number_format((float) ($row['total_due_amount'] ?? 0), 2),
                    ];
        ...
        $pageGroups = $this->buildModalBillRowsForPageSummaries($pageSummaries, $dateFrom, $dateTo);
        if ((! $forPrint || $pageGroups->count() <= 100) && ! $skipLifetimeDue) {
    ...
    private function applySellingVoucherDateRangeItemIssueDateFilter($query, ?string $dateFromYmd, ?string $dateToYmd): void
    {
        ...
        if ($dateFromYmd && $dateToYmd) {
            $query->whereIn($table . '.id', function ($sub) use ($dateFromYmd, $dateToYmd) {
                $sub->select('sv_date_range_report_id')
                    ->from('sv_date_range_report_items')
                    ->whereBetween('issue_date', [$dateFromYmd, $dateToYmd])
                    ->distinct();
            });
    ...
    /**
     * My Mess Bills page sends my_bills=1 so details/print match the invoiced-only list
     * (mess staff may also have admin mess permissions).
     */
    private function isMyMessBillInvoicedOnlyRequest(Request $request): bool
    {
        return $request->boolean('my_bills');
    }
