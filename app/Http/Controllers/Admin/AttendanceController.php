    ...
    // }
      ...
      public function OTmarkAttendanceView(Request $request, $group_pk, $course_pk, $timetable_pk, $student_pk){
        ...
        $archivedCourses = CourseMaster::join('student_master_course__map', 'student_master_course__map.course_master_pk', '=', 'course_master.pk')
           ...
           return view('admin.attendance.ot-student-view', compact('group_pk', 'course_pk', 'timetable_pk', 'student_pk','sessions','maunalSessions','course','archivedCourses',
                'student'));
    }
public function OTmarkAttendanceData(
    Request $request
 ) {

     $archive_mode = $request->archive_mode;
     $student_pk = $request->student_pk;
     $filterCourse = $request->course_pk;
    $archivedCourses = [];
        if ($archive_mode == '') {
            $archivedCourses = CourseMaster::join('student_master_course__map', 'student_master_course__map.course_master_pk', '=', 'course_master.pk')
                ->where('student_master_course__map.student_master_pk', $student_pk)
...
$query = CourseGroupTimetableMapping::with([
        ...
        ->where('group_pk', $request->group_pk)
