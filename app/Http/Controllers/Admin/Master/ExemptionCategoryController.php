    ...
    try {
           //dd($request); 
            ExemptionMedicalSpecialityMaster::create([
                ...
                'active_inactive' => $request->status ?? 1,
            ]);
...
public function medicalSpecialityEdit($id)
{
 ...
 public function MedSpecExupdate(Request $request)
    {
        $table = 'exemption_medical_speciality_master';
        try {
            DB::table($table)->where('pk', $request->pk)->update(['speciality_name' => $request->speciality_name,
        'active_inactive' => $request->status
        ]);
            return redirect()->back()->with('success', 'Exemption categories updated successfully');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Exemption categories not correct');
