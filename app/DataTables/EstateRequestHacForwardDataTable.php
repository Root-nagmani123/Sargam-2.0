...
class EstateRequestHacForwardDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('forward', function ($row) {
                $url = route('admin.estate.request-for-estate.forward', ['id' => $row->pk]);
                return '<a href="javascript:void(0);" class="btn btn-sm btn-primary btn-forward-to-allotment" data-url="' . e($url) . '" data-req-id="' . e($row->req_id ?? '') . '" title="Forward to Allotment Team">
    ...
    public function query(EstateHomeRequestDetails $model): QueryBuilder
    {
        return $model->newQuery()
            ...
            ->where('estate_home_request_details.f_status', 0)
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('forward')->title('FORWARD')->addClass('text-center')->orderable(false)->searchable(false)->width('120px'),
        ];
    }

    protected function filename(): string
    {
        return 'HacForward_' . date('YmdHis');
    }
}
