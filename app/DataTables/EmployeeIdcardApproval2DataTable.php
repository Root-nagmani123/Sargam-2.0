...
/**
 * "Requested ID Card" (Approval II) list — 4 tabs (new / for_approval / issued / rejected) on
 * one page. Reuses EmployeeIDCardApprovalController::buildApproval2Lists() (the exact same
 ...
 */
class EmployeeIdcardApproval2DataTable extends DataTable
{
    public const LISTING_CACHE_EPOCH_KEY = 'employee_idcard_approval2_list_epoch';

    public static function bumpListingCacheEpoch(): void
    {
        DataTableRedisCache::bumpListEpoch(self::LISTING_CACHE_EPOCH_KEY, 'EmployeeIdcardApproval2DataTable');
    }

    /**
     * Server-side JSON for one tab of the Approval II list. .env: EMPLOYEE_IDCARD_APPROVAL2_CACHE_*.
     */
    public function ajax(): JsonResponse
    {
        ...
        return DataTableRedisCache::serveCachedAjax(
            $request,
            'employee_idcard_approval2_dt:v1:',
            self::LISTING_CACHE_EPOCH_KEY,
            [
                'enabled' => 'EMPLOYEE_IDCARD_APPROVAL2_CACHE_ENABLED',
                'seconds' => 'EMPLOYEE_IDCARD_APPROVAL2_CACHE_SECONDS',
            ],
            'EmployeeIdcardApproval2DataTable',
            fn () => parent::ajax(),
            [
    ...
    public function query(Request $request): Collection
    {
        ...
        $lists = $controller->buildApproval2Lists($request);
     ...
     * for approvalStage = 2 exactly (including the Issued-tab "card print" checkbox form).
     */
    private function renderActionsColumn(object $row): string
    {
        ...
        $viewUrl = route('admin.security.employee_idcard_approval.show', ['id' => $encryptedId, 'stage' => 2]);
        ...
        if ($status !== 'Pending') {
            ...
            if ($status === 'Approved' && empty($row->id_card_physical_print_done ?? false)) {
                $token = csrf_token();
                $checkboxId = 'archive-check-' . md5((string) $encryptedId);
                $html .= '<form action="' . e(route('admin.security.employee_idcard_approval.markGenerated', $encryptedId)) . '" method="POST" class="d-inline mt-1 archive-check-form">'
                    . '<input type="hidden" name="_token" value="' . e($token) . '">'
                    . '<div class="form-check form-check-inline m-0">'
                    . '<input class="form-check-input archive-check-input" type="checkbox" id="' . e($checkboxId) . '">'
                    . '<label class="form-check-label small text-muted" for="' . e($checkboxId) . '">card print</label>'
                    . '</div></form>';
            }
            ...
            $approveRoute = route('admin.security.employee_idcard_approval.approve2', $encryptedId);
            $rejectRoute = route('admin.security.employee_idcard_approval.reject2', $encryptedId);
            $token = csrf_token();
    ...
    {
        return 'EmployeeIdcardApproval2_' . date('YmdHis');
    }
}
