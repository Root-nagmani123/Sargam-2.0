    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) use ($hasStatus) {
                ...
                // Icon-over-label Edit (opens the AJAX modal via .openEditCardType) · toggle · Delete (§3b).
                $editBtn = '<a href="' . $editUrl . '" class="ct-act ct-act--edit openEditCardType" title="Edit" aria-label="Edit card type">'
                ...
                if ($canDelete) {
                    $deleteBtn = '<form action="' . $deleteUrl . '" method="POST" class="d-inline m-0 cardtype-delete-form" '
                        . 'onsubmit="return confirm(\'Delete this Card Type?\');">'
                        . csrf_field() . method_field('DELETE')
                        . '<button type="submit" class="ct-act ct-act--del" title="Delete" aria-label="Delete card type">'
                        . '<i class="bi bi-trash3" aria-hidden="true"></i><span>Delete</span></button></form>';
                } else {
                    $deleteBtn = '<button type="button" class="ct-act ct-act--del is-disabled" disabled '
