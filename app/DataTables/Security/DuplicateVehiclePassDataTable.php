...
class DuplicateVehiclePassDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function ($row) {
                // Soft workflow badge (§3b look): Approved / Rejected / Issued / Pending.
                $text = $row->status_text;
                $cls = match ($text) {
                    'Approved' => 'bg-success-subtle',
                    'Rejected' => 'bg-danger-subtle',
                    'Issued'   => 'bg-info-subtle',
                    default    => 'bg-warning-subtle',
                };

                return '<span class="status-pill badge ' . $cls . '">' . e($text) . '</span>';
            })
            ->addColumn('action', function ($row) {
                // Icon-over-label actions (§3b). Edit/Delete ONLY while Pending
                // (workflow guard — preserved exactly).
                ...
                $html = '<div class="d-inline-flex align-items-center justify-content-center gap-3" role="group" aria-label="Row actions">'
                    . '<a href="' . $viewUrl . '" class="dvp-act dvp-act--view" title="View" aria-label="View request">'
                ...
                if ((int) $row->vech_card_status === VehiclePassDuplicateApplyTwfw::STATUS_PENDING) {
                    ...
                    $html .= '<a href="' . $editUrl . '" class="dvp-act dvp-act--edit" title="Edit" aria-label="Edit request">'
                        . '<i class="bi bi-pencil-square" aria-hidden="true"></i><span>Edit</span></a>'
                        . '<form action="' . $deleteUrl . '" method="POST" class="d-inline m-0 dvp-delete-form" '
                        . 'onsubmit="return confirm(\'Delete this request?\');">'
                        . csrf_field() . method_field('DELETE')
                        . '<button type="submit" class="dvp-act dvp-act--del" title="Delete" aria-label="Delete request">'
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('request_date')->title('Request Date')->searchable(false)->orderable(false)->addClass('text-center'),
