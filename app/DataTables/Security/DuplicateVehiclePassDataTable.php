...
class DuplicateVehiclePassDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('employee_name', fn ($row) => e($row->employee_name ?? '--'))
            ->addColumn('vehicle_pass_no', fn ($row) => e($row->vehicle_primary_pk ?? '--'))
            ...
            ->addColumn('request_date', fn ($row) => $row->created_date ? $row->created_date->format('d-m-Y') : '--')
            ->addColumn('status', function ($row) {
                ...
                return '<span class="badge rounded-1 ' . $badge . '">' . e($text) . '</span>';
            })
            ->addColumn('action', function ($row) {
                $id = encrypt($row->vehicle_tw_pk);
                $viewUrl = route('admin.security.duplicate_vehicle_pass.show', $id);
                ...
                if ((int) $row->vech_card_status === VehiclePassDuplicateApplyTwfw::STATUS_PENDING) {
                    $editUrl = route('admin.security.duplicate_vehicle_pass.edit', $id);
                    $deleteUrl = route('admin.security.duplicate_vehicle_pass.delete', $id);
                    ...
                    $html .= '<a href="' . $editUrl . '" class="programme-action-btn" title="Edit" aria-label="Edit request">'
                        ...
                        . '<form action="' . $deleteUrl . '" method="POST" class="d-inline dvp-delete-form" '
            ...
            ->filterColumn('vehicle_pass_no', function ($query, $keyword) {
                $query->where('vehicle_primary_pk', 'like', "%{$keyword}%");
            })
            ...
            // explicit SQL expression. employee_name and vehicle_type_name come from
            // relations (no such column on this table), hence the correlated subqueries.
            ->orderColumn('employee_name', 'coalesce((select nullif(trim(concat(coalesce(e.first_name, \'\'), \' \', coalesce(e.last_name, \'\'))), \'\') '
                . 'from employee_master e where e.pk = `vehicle_pass_duplicate_apply_TWFW`.emp_master_pk), '
                . '`vehicle_pass_duplicate_apply_TWFW`.employee_id_card) $1')
            ->orderColumn('vehicle_pass_no', '`vehicle_pass_duplicate_apply_TWFW`.vehicle_primary_pk $1')
            ->orderColumn('vehicle_type_name', '(select vt.vehicle_type from sec_vehicle_type vt '
                . 'where vt.pk = `vehicle_pass_duplicate_apply_TWFW`.vehicle_type) $1')
            ->orderColumn('vehicle_no', '`vehicle_pass_duplicate_apply_TWFW`.vehicle_no $1')
            ->orderColumn('request_date', '`vehicle_pass_duplicate_apply_TWFW`.created_date $1')
            ->orderColumn('status', '`vehicle_pass_duplicate_apply_TWFW`.vech_card_status $1')
            ->rawColumns(['document', 'status', 'action']);
    }

    /**
     * Get query source of dataTable. Scoped to the passes created by the current user
     * (mirrors DuplicateVehiclePassController@index).
     *
     * @param \App\Models\VehiclePassDuplicateApplyTwfw $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(VehiclePassDuplicateApplyTwfw $model): QueryBuilder
    {
        $user = Auth::user();
        ...
        // The default sort (request date desc) lives in html() as the `order` parameter.
    ...
    {
        return $this->builder()
            ->setTableId('duplicateVehPass-table')
            ...
            ->parameters([
                ...
                // Default sort: newest request first (Request Date, column index 6).
                'order'        => [[6, 'desc']],
                'language'     => [
                    'search'            => '',
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('request_date')->title('Request Date')->searchable(false)->orderable(true)->addClass('text-center'),
    ...
    {
        return 'DuplicateVehiclePass_' . date('YmdHis');
    }
}
