    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('out_time', function ($row) {
                ...
                return '<span class="status-pill badge bg-success-subtle">Active</span>';
            })
            ->addColumn('action', function ($row) {
                ...
                $html = '<div class="d-inline-flex align-items-center justify-content-center gap-3" role="group" aria-label="Row actions">'
                    . '<a href="' . $viewUrl . '" class="vis-act vis-act--view" title="View" aria-label="View visitor pass">'
                ...
                if (! $row->out_time) {
                    ...
                    $html .= '<form action="' . $checkoutUrl . '" method="POST" class="d-inline m-0" '
                        . 'onsubmit="return confirm(\'Mark visitor as checked out?\');">'
                        . csrf_field()
                        . '<button type="submit" class="vis-act vis-act--checkout" title="Check Out" aria-label="Check out visitor">'
                        . '<i class="bi bi-box-arrow-right" aria-hidden="true"></i><span>Check Out</span></button></form>'
                        . '<a href="' . $editUrl . '" class="vis-act vis-act--edit" title="Edit" aria-label="Edit visitor pass">'
                        . '<i class="bi bi-pencil" aria-hidden="true"></i><span>Edit</span></a>';
                }

                $html .= '<form action="' . $deleteUrl . '" method="POST" class="d-inline m-0 vis-delete-form" '
                    . 'onsubmit="return confirm(\'Delete this visitor pass?\');">'
                    . csrf_field() . method_field('DELETE')
                    . '<button type="submit" class="vis-act vis-act--del" title="Delete" aria-label="Delete visitor pass">'
