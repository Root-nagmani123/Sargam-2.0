...
class CardSubTypeMasterDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) use ($hasStatus) {
                ...
                // Icon-over-label Edit (opens the AJAX modal via .openEditSubType) · toggle · Delete (§3b).
                $editBtn = '<a href="' . $editUrl . '" class="st-act st-act--edit openEditSubType" title="Edit" aria-label="Edit sub type">'
                ...
                if ($canDelete) {
                    $deleteBtn = '<form action="' . $deleteUrl . '" method="POST" class="d-inline m-0 subtype-delete-form" '
                        . 'onsubmit="return confirm(\'Delete this Sub Type mapping?\');">'
                        . csrf_field() . method_field('DELETE')
                        . '<button type="submit" class="st-act st-act--del" title="Delete" aria-label="Delete sub type">'
                        . '<i class="bi bi-trash3" aria-hidden="true"></i><span>Delete</span></button></form>';
                } else {
                    $deleteBtn = '<button type="button" class="st-act st-act--del is-disabled" disabled '
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('card_type')->title('Card Type')->orderable(false),
            Column::computed('employee_category')->title('Employee Category')->searchable(true)->orderable(false),
            Column::make('sub_type')->title('Sub Type')->orderable(false),
