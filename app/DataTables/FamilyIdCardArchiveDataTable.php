...
/**
 * Archive (id_status IN (2,3)) tab of /admin/family-idcard.
 *
 ...
 */
class FamilyIdCardArchiveDataTable extends DataTable
{
    /**
    ...
    /**
     * Server-side JSON for the Archive list. .env: FAMILY_IDCARD_ARCHIVE_DATATABLE_CACHE_*.
     */
    public function ajax(): JsonResponse
    {
        ...
        return DataTableRedisCache::serveCachedAjax(
            $request,
            'admin_family_idcard_archive_dt:v1:',
            self::LISTING_CACHE_EPOCH_KEY,
            [
                'enabled' => 'FAMILY_IDCARD_ARCHIVE_DATATABLE_CACHE_ENABLED',
                'seconds' => 'FAMILY_IDCARD_ARCHIVE_DATATABLE_CACHE_SECONDS',
            ],
            'FamilyIdCardArchiveDataTable',
            fn () => parent::ajax(),
            [
    ...
    public function query(Request $request): Collection
    {
        ...
        return FamilyIDCardRequestController::buildFamilyIdcardGroupedListFromRows($archiveRows, $cardType);
    }
    ...
    /**
     * Mirrors the Archive tab @forelse block in resources/views/admin/family_idcard/index.blade.php.
     */
    private function renderActionsColumn(object $group): string
    {
        ...
        if ((int) ($group->id_status ?? 1) === 3) {
            $restoreUrl = route('admin.family_idcard.restore', $group->first_id);
            $token = csrf_token();

            $html .= '<form action="' . e($restoreUrl) . '" method="POST" class="d-inline" onsubmit="return confirm(\'Restore this request?\');">'
                . '<input type="hidden" name="_token" value="' . e($token) . '">'
                . '<button type="submit" class="btn  btn-outline-success bg-transparent border-0 text-primary p-0" title="Restore">'
                . '<i class="material-icons material-symbols-rounded" style="font-size:18px;">restore</i></button>'
                . '</form>';
        }
    ...
    {
        return $this->builder()
            ->setTableId('familyIdcardArchiveTable')
            ->addTableClass('table table-hover mb-0 align-middle family-idcard-table')
    ...
    {
        return 'FamilyIdCardArchive_' . date('YmdHis');
    }
}
