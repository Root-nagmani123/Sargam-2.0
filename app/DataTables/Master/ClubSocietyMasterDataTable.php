...
class ClubSocietyMasterDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('club_society_name', fn ($row) => e($row->club_society_name ?? '-'))
            ->addColumn('action', function ($row) {
                $encryptedPk = encrypt($row->pk);

                $editBtn = '<button type="button" class="cs-action-btn cs-edit-btn" aria-label="Edit club/society"'
                    . ' data-id="' . $encryptedPk . '"'
                    . ' data-name="' . e($row->club_society_name) . '"'
                    . ' data-status="' . (int) $row->active_inactive . '">'
                    . '<i class="bi bi-pencil" aria-hidden="true"></i>'
                    . '<span>Edit</span>'
                    . '</button>';

                $deleteBtn = '<button type="button" class="cs-action-btn cs-action-btn--danger cs-delete-btn" aria-label="Delete club/society"'
                    . ' data-id="' . $encryptedPk . '"'
                    . ' data-name="' . e($row->club_society_name) . '">'
            ...
            ->filterColumn('club_society_name', function ($query, $keyword) {
                $query->where('club_society_name', 'like', "%{$keyword}%");
            })
            ->rawColumns(['club_society_name', 'action']);
    }

    /**
     * Get query source of dataTable.
     */
    public function query(ClubSocietyMaster $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('clubsocietymaster-table')
    ...
    {
        return 'ClubSocietyMaster_' . date('YmdHis');
    }
}
