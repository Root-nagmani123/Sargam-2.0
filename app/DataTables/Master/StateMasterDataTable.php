...
class StateMasterDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('state_name', fn($row) => $row->state_name ?? '-')
            ...
            ->addColumn('action', function ($row) {
                $edit = route('master.state.edit', $row->pk);
                ...
                if ($row->active_inactive == 1) {
                    return '
                        ...
                        <button type="button" class="btn btn-sm btn-outline-secondary" disabled title="Cannot delete active state">
                            <i class="material-icons" style="font-size:16px;">delete</i>
                        </button>';
                }

                $delete = route('master.state.delete', $row->pk);
            ...
            ->filterColumn('state_name', function ($query, $keyword) {
                $query->where('state_name', 'like', "%{$keyword}%");
            })
            ->rawColumns(['state_name', 'status', 'action']);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('state_name')->title('State Name')->orderable(false)->addClass('text-center'),
