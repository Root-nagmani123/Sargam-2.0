...
class StudentAttendanceListDataTable extends DataTable
{
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('mdo_duty')->title('MDO Duty')->addClass('text-nowrap mark-att-mdo-col')->orderable(false)->searchable(false),
            Column::make('escort_duty')->title('Escort/ Modular Duty')->addClass('text-nowrap mark-att-escort-col')->orderable(false)->searchable(false),
            Column::make('row_action')->title('Action')->addClass('text-nowrap mark-att-action-col')->orderable(false)->searchable(false),
        ];
    }
    ...
    protected function renderRadioHiddenOnly($row, int $value): string
    {
        ...
        // Escort/Moderator (value 5) is NEVER auto-checked. An OT on escort/modular
        // duty is treated as "Present" — that Present default is set in renderRadioGroup.
        // Because every radio for a student shares the same name, a checked escort radio
        // (rendered after the Present radio) would win in the browser and knock out the
        // Present default, so this hidden radio must stay unchecked.
        return "<input type='radio' class='form-check-input' name='student[{$studentId}]' value='{$value}' id='student[{$studentId}][{$value}]'>";
    }
