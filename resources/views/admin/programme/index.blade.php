...
$(document).ready(function() {
    ...
    setTimeout(function() {
        ...
        // ── Custom SweetAlert: Activate / Deactivate Course ──
        $(document).on('click', '.programme-status-toggle-btn', function(e) {
            e.preventDefault();
            var $btn = $(this);
            var tableName = $btn.data('table');
            var columnName = $btn.data('column');
            var itemId = $btn.data('id');
            var currentStatus = parseInt($btn.data('status'));
            var newStatus = currentStatus === 1 ? 0 : 1;

            var isActivating = newStatus === 1;
            var iconColor = isActivating ? '#198754' : '#daa520';
            var iconName = isActivating ? 'info' : 'priority_high';
            var title = isActivating ? 'Activate Course?' : 'Deactivate Course?';
            var text = isActivating ? 'Are you sure you want to activate this course?' :
                'Are you sure you want to deactivate this course?';
            var confirmText = isActivating ? 'Yes, Activate' : 'Yes, Deactivate';
            var cancelText = isActivating ? 'Cancel, Keep it deactive' :
                'Cancel, Keep it active';

            Swal.fire({
                html: '<div style="text-align:center;">' +
                    '<div style="margin:0 auto 16px;width:72px;height:72px;border-radius:50%;border:4px solid ' +
                    iconColor +
                    ';display:flex;align-items:center;justify-content:center;">' +
                    '<span class="material-icons material-symbols-rounded" style="font-size:36px;color:' +
                    iconColor + ';">' + iconName + '</span></div>' +
                    '<h4 style="font-weight:700;margin-bottom:4px;">' + title +
                    '</h4>' +
                    '<p style="color:#6c757d;margin:0;">' + text + '</p></div>',
                showCancelButton: true,
                confirmButtonText: confirmText,
                cancelButtonText: cancelText,
                customClass: {
                    cancelButton: 'btn btn-outline-dark border-2 fw-semibold px-4',
                    confirmButton: 'btn btn-dark fw-semibold px-4',
                    actions: 'gap-3 mt-2'
                },
                buttonsStyling: false,
                reverseButtons: true,
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function() {
                            Swal.fire('Error', 'Status update failed',
                                'error');
                        }
                    });
                }
