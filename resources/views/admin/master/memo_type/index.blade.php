...
<div class="container-fluid mmt-master-page">
    ...
    <div class="card mmt-dt-card border-0 shadow-sm rounded-3 overflow-hidden">
        <div class="card-body p-3 p-md-4">
            <div
                class="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-end gap-3 mb-4">
                <div id="mmtDtSearch" class="programme-dt-search ms-lg-auto" data-dt-search-for="memotypemaster-table">
                </div>
            </div>

            <div class="programme-dt-panel mmt-dt-scroll">
                {!! $dataTable->table(['class' => 'table align-middle mb-0 w-100 programme-dt-table']) !!}
                <div id="mmtDtFooter"
...
<div class="modal fade mmt-form-modal" id="mmtMemoTypeModal" tabindex="-1" aria-labelledby="mmtMemoTypeModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered mmt-modal-dialog">
        <div class="modal-content cgt-form-modal border-0 shadow-lg rounded-4">
            ...
            <div class="modal-body pt-4 pb-2">
                <form id="memoTypeForm" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="memo_type_name" id="memo_type_name" class="form-control rounded-3"
                    ...
                    <div class="mb-0">
                        ...
                        <div class="mmt-file-upload rounded-3">
                            ...
                            <input type="file" name="memo_doc_upload" id="memo_doc_upload" class="mmt-file-upload-input"
                                accept=".pdf,.doc,.docx">
                        </div>
                        <small class="text-danger d-none mt-1 d-block" id="memo_doc_upload_error"></small>
                        <div id="mmt_existing_doc_wrap" class="mt-2 d-none">
                            <a href="#" target="_blank" rel="noopener noreferrer" id="mmt_existing_doc_link"
            ...
            <div class="modal-footer border-top border-0 gap-2 justify-content-end pt-3 pb-4 px-4">
                <button type="button" class="btn btn-outline-primary rounded-3 px-4 py-2"
                    data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary rounded-3 px-4 py-2" id="mmtFormSubmit">Create Memo
                    Type</button>
            </div>
        </div>
    </div>
...
$(function() {
    ...
    function iconOnlyBtn($btn, iconClass, extraClass) {
        $btn.removeClass(
            'btn btn-sm btn-outline-primary btn-outline-danger btn-outline-secondary d-inline-flex align-items-center gap-1'
            );
    ...
    function buildToggleControl($toggle) {
        ...
        $label.append(
            '<i class="bi bi-toggle-off mmt-toggle-icon mmt-toggle-icon--off" aria-hidden="true"></i>');
    ...
    function decorateMmtRows() {
        $(tableSelector + ' tbody tr').each(function() {
            ...
            const badgeClass = isActive ? 'programme-status-badge--active' :
                'programme-status-badge--inactive';

            $statusCell.empty().append(
                $('<span>', {
                    class: 'badge rounded-pill programme-status-badge mmt-status-badge ' +
                        badgeClass,
            ...
            const $disabledDelete = $actionCell.find('button[disabled]').not('.deleteBtn').first()
                .detach();
            ...
            error: function(xhr) {
                if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                    ...
                    if (errors.memo_type_name) {
                        $form.find('#memo_type_name_error').text(errors.memo_type_name[0])
                            .removeClass('d-none');
                        name.addClass('is-invalid');
                    }
                    if (errors.active_inactive) {
                        $form.find('#active_inactive_error').text(errors.active_inactive[0])
                            .removeClass('d-none');
                        status.addClass('is-invalid');
                    }
                    if (errors.memo_doc_upload) {
                        $form.find('#memo_doc_upload_error').text(errors.memo_doc_upload[0])
                            .removeClass('d-none');
                    }
                } else {
                    Swal.fire('Error', xhr.responseJSON?.message ||
                        'Server error or session expired', 'error');
                }
            }
        });
    });
