...
<head>
    <meta charset="utf-8">
    <title>Define Club/ Society — LBSNAA</title>
...
<body onload="window.print();">
    ...
    @include('admin.issue_management.partials.export_print_header', [
        'title'      => 'Define Club/ Society',
        'exportDate' => $exportDate,
        'filterLine' => null,
        'total'      => $rows->count(),
    ])

    {{-- Same columns as ClubSocietyMasterExport so the sheet and the printout
         can't drift apart. --}}
    <table class="ic-print-table">
        <thead>
            <tr>
                <th class="col-sno">S. No.</th>
                <th class="col-name">Club/Society/Association</th>
        ...
        <tbody>
            @forelse ($rows as $index => $row)
                <tr>
                    ...
                    <td class="col-name">{{ $row->club_society_name }}</td>
                    <td class="col-status">{{ (int) $row->active_inactive === 1 ? 'Active' : 'Inactive' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="3" class="ic-print-empty">No clubs / societies to print</td>
                </tr>
            @endforelse
        </tbody>
    </table>
