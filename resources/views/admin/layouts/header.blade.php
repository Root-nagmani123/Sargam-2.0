@php
    ...
    if (request()->routeIs('admin.dashboard') || request()->routeIs('admin.dashboard.*')) {
        ...
        // Personal estate from Home sidebar (?scope=self): keep Home tab + content section; full estate admin stays on Setup.
        $estateHomeSelfContext = request('scope') === 'self'
            && (hasRole('Admin') || hasRole('Super Admin') || hasRole('Estate'));
        $activeNavTab = $estateHomeSelfContext ? '#home' : '#tab-setup';
    } elseif (
