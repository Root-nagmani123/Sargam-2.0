...
<x-fonts-sargam />
{{-- UX4G Design System (Phase 2). Loaded FIRST and confined to cascade layers
     (@layer ux4g / tokens), so the existing UNLAYERED theme below still wins every
     rule — verified zero visual change. The migration reveals UX4G progressively by
     removing the existing overrides component-by-component in later phases.
       ux4g-vendor.css : tamed UX4G (1,668 !important stripped) in @layer ux4g
       ux4g-app.css    : brand tokens (LBSNAA navy + WCAG fixes) in @layer tokens
     Both are generated by `npm run build:ux4g`. See docs/UX4G-Decision-Note-R1-R2-R3.md --}}
<link rel="stylesheet" href="{{ asset('css/ux4g-vendor.css') }}?v={{ @filemtime(public_path('css/ux4g-vendor.css')) ?: time() }}">
