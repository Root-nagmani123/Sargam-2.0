<tr data-id="{{ $field->id }}">
    ...
    <td class="fc-fb-actions-col">
        <div class="fc-fb-actions">
            <button type="button" class="btn btn-sm btn-outline-primary py-0 px-1" onclick="editField({{ json_encode($field) }})" title="Edit">
                <i class="bi bi-pencil"></i>
            </button>
            <button type="button" class="btn btn-sm btn-outline-secondary py-0 px-1" onclick="moveField({{ $field->id }}, 'up')" title="Move Up">
                <i class="bi bi-arrow-up"></i>
            </button>
            <button type="button" class="btn btn-sm btn-outline-secondary py-0 px-1" onclick="moveField({{ $field->id }}, 'down')" title="Move Down">
            ...
            @else
                <form method="POST" action="{{ route('fc-reg.admin.form-builder.field.delete', $field) }}" class="fc-fb-actions__form" onsubmit="return confirm('Delete this field?')">
                    @csrf @method('DELETE')
