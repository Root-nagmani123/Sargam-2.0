                    ...
                    <tbody>
                        ...
                        @foreach ($groupedForms[null] ?? [] as $parent)
                            ...
                            <tr class="parent-row" data-id="{{ $parent->id }}" style="cursor:pointer;">
                                ...
                                <td>
                                    <a href="{{ route('forms.courseList', $parent->id) }}" class="btn btn-sm btn-success">View</a>
                                </td>
                                <td>
                                    <a href="{{ route('forms.show', $parent->id) }}" class="btn btn-sm btn-info">Preview</a>
                                </td>
                                <td>
                                    <a href="{{ route('forms.fc_edit', $parent->id) }}" class="btn btn-sm btn-warning">Edit Fields</a>
                                </td>
                                <td>
                            ...
                            @if (isset($groupedForms[$parent->id]))
                                @foreach ($groupedForms[$parent->id] as $index => $child)
                                    <tr class="child-row bg-light" data-parent="{{ $parent->id }}" style="display:none;">
                                        ...
                                        <td>
                                            <a href="{{ route('forms.courseList', $child->id) }}" class="btn btn-sm btn-success">View</a>
                                        </td>
                                        <td>
                                            <a href="{{ route('forms.show', $child->id) }}" class="btn btn-sm btn-info">Preview</a>
                                        </td>
                                        <td>
                                            <a href="{{ route('forms.fc_edit', $child->id) }}" class="btn btn-sm btn-warning">Edit Fields</a>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input toggle-visible-switch"
                                                       type="checkbox" data-id="{{ $child->id }}" {{ $child->visible ? 'checked' : '' }}>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            ...
                                            @if ($index < count($groupedForms[$parent->id]) - 1)
                                                <form action="{{ route('forms.movedown', $child->id) }}" method="POST" class="d-inline">
                                                    @csrf
...
$(document).ready(function() {
    ...
    var table = $('#zero_config').DataTable({
        "order": [],
        "columnDefs": [
            { "orderable": false, "targets": [0,5,6,7,8,9,10] }
        ]
    });
