...
@section('setup_content')
                        ...
                        <form action="{{ route('student.enrollment.import') }}" method="POST"
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        <div class="alert"
                                            style="background:#e9f5ff; border:1px solid #b6e0fe; color:#055160;">
                                            <h6 class="alert-heading"><i class="fas fa-info-circle me-2"></i>Import
                                                Instructions:</h6>
                                            ...
                                            <p class="mt-2 mb-0"><strong>Note:</strong> Export first, edit OT Code column,
                                                then import the same file.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="form-text-dark">
                                        <i class="fas fa-exclamation-triangle me-1"></i>Do not modify student_master_pk or
                                        course_master_pk columns
                                    </div>
                                </div>
                                    ...
                                    <div class="alert alert-danger mt-3">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <h6 class="alert-heading mb-0">
                                                <i class="fas fa-exclamation-circle me-2"></i>Import Errors
    ...
    @section('scripts')
        ...
        <script type="text/javascript" src="https://cdn.datatables.net/responsive/2.5.0/js/responsive.bootstrap5.min.js">
        </script>
        ...
        <script>
            ...
            $('#importForm').on('submit', function(e) {
                ...
                if (!validExtensions.exec(fileName)) {
                    e.preventDefault();
                    showAlert('error', 'Please upload only Excel or CSV files (.xlsx, .xls, .csv)');
                    return false;
                }
                ...
                showAlert('info', 'Processing import. Please do not close this window.');
            });

            // Reset import button when modal is closed
            $('#importModal').on('hidden.bs.modal', function() {
                $('#importSubmitBtn').prop('disabled', false).html(
                    '<i class="fas fa-upload me-1"></i> Import to OT List');
                $('#importForm')[0].reset();
            });
            ...
            function showAlert(type, message) {
                ...
                const alertHtml = `
        <div class="alert ${alertClass} import-alert alert-dismissible fade show mt-2" role="alert">
            <i class="fas fa-${icon} me-2"></i>${message}
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    `;

                $('#importModal .modal-body').prepend(alertHtml);
            }
        </script>
