    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <!-- Export Buttons (will be updated dynamically) -->
...
<script>
    $(document).ready(function() {
        ...
        function updateExportUrls(courseId) {
            const basePdfUrl = '{{ route("enrollment.exportEnrolled", ["type" => "pdf"]) }}';
            const baseExcelUrl = '{{ route("enrollment.exportEnrolled", ["type" => "excel"]) }}';
            
            const pdfUrl = courseId ? `${basePdfUrl}&course_id=${courseId}` : basePdfUrl;
            const excelUrl = courseId ? `${baseExcelUrl}&course_id=${courseId}` : baseExcelUrl;
        ...
        enrolledModalEl.addEventListener('show.bs.modal', function() {
            // Initialize modal with selected course from main form or empty
            const mainCourse = $('#course_master_pk').val();
            if (mainCourse) {
                $('#modalCourseSelect').val(mainCourse).trigger('change');
                currentCourseFilter = mainCourse;
            } else {
                $('#modalCourseSelect').val('').trigger('change');
        ...
        // Course selection change in modal
        ...
        function updateDisplayInfo() {
            ...
            if (currentCourseFilter) {
                ...
                // For "All Courses", pass empty or don't show export buttons
                $('#exportButtons').hide();
            }
        }
