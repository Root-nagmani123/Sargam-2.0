...
<div class="container-fluid cal-create-event-page py-2">
    ...
    <x-breadcrum title="Add Event" :items="[
        ['label' => 'Home', 'url' => url('/')],
        ['label' => 'Time Table'],
    ...
    <form id="createEventForm" method="POST" action="{{ route('calendar.event.store') }}" novalidate>
        @csrf
                ...
                <div class="row g-3">
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="date" name="start_datetime" id="start_datetime"
                            class="form-control" required value="{{ old('start_datetime', $prefillDate) }}">
                    </div>
                    <div class="col-md-6 col-lg-4">
                        ...
                        <select name="Course_name" id="Course_name" class="form-select" required>
                            ...
                            @foreach($courseMaster as $course)
                                <option value="{{ $course->pk }}" @selected(old('Course_name') == $course->pk)>{{ $course->course_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 col-lg-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="subject_module" id="subject_module" class="form-select" required>
                            ...
                            @foreach($subjects as $subject)
                                <option value="{{ $subject->pk }}" @selected(old('subject_module') == $subject->pk)>{{ $subject->module_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="sector" id="sector" class="form-select">
                            ...
                            @foreach($sectors as $sector)
                                <option value="{{ $sector->pk }}" @selected(old('sector') == $sector->pk)>{{ $sector->sector_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="vanue" id="vanue" class="form-select" required>
                            ...
                            @foreach($venueMaster as $loc)
                                <option value="{{ $loc->venue_id }}" @selected(old('vanue') == $loc->venue_id)>{{ $loc->venue_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <section class="card cal-ce-card border-0 shadow-sm rounded-3 mb-3">
            <div class="card-body p-4">
                ...
                <div id="facultyRows">
                    {{-- rows injected by JS (and rebuilt from old input on validation error) --}}
                </div>
            ...
            <div class="card-body p-4">
                    ...
                    <div class="col-md-12" id="shiftSelectWrap">
                        ...
                        <select name="shift" id="shift" class="form-select">
                            ...
                            @foreach($classSessionMaster as $shift)
                                <option value="{{ $shift->shift_time }}" @selected(old('shift') == $shift->shift_time)>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="time" name="break_start_time" id="break_start_time" class="form-control" value="{{ old('break_start_time') }}">
                    </div>
                    <div class="col-md-6">
                        <label for="break_end_time" class="form-label">Break End Time</label>
                        <input type="time" name="break_end_time" id="break_end_time" class="form-control" value="{{ old('break_end_time') }}">
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="card-body p-4">
                ...
                <div class="cal-ce-option-row d-flex align-items-center justify-content-between border rounded-2 px-3 py-2">
                    ...
                    <div class="form-check form-switch mb-0">
                        <input class="form-check-input" type="checkbox" id="bio_attendanceCheckbox" name="bio_attendanceCheckbox" value="1" @checked(old('bio_attendanceCheckbox'))>
        ...
        <div class="d-flex justify-content-end gap-2 mb-4">
            ...
            <button type="submit" class="btn btn-primary rounded-2 px-4">Add Event</button>
        </div>
    </form>
</div>

{{-- Faculty row template (cloned by JS) --}}
...
<style>
    ...
    /* Choices.js: match Bootstrap form-control sizing/appearance */
...
(function () {
    ...
    // Set a Choices-managed (or plain) select's value, keeping the UI in sync.
    ...
    // Replace a Choices select's options (used for dependent dropdowns).
    ...
    function addFacultyRow(preset) {
        ...
        // Searchable dropdowns for the row's selects.
    ...
    // Seed faculty rows: restore old input after a validation error, else one empty row.
    (function seedFacultyRows() {
        ...
        if (!facultyRows.querySelector('[data-faculty-row]')) {
            addFacultyRow();
        }
    })();
    ...
    function renderGroupNames(groups, preselectNames) {
        ...
        const selected = (preselectNames && typeof preselectNames === 'object')
            ? Object.values(preselectNames).map(String) : [];
    ...
    // Searchable dropdowns for all static selects.
    ['Course_name', 'group_type', 'subject_module', 'subject_name', 'sector', 'vanue', 'shift']
        .forEach((id) => initChoices(document.getElementById(id)));

    // Restore dependent dropdowns after a validation error.
    if (courseSelect.value) {
        loadGroupTypes(oldData.group_type, oldData.type_names);
    }
    if (moduleSelect.value) {
        loadSubjectNames(oldData.subject_name);
    }
    ...
    /* ---------------- Download form (print) ---------------- */
    document.getElementById('btnDownloadForm')?.addEventListener('click', () => window.print());
})();
</script>
@endpush
