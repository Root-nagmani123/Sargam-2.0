{{-- Dedicated OT (Officer Trainee / Student-OT) calendar view. Always renders in the timetable layout. --}}
...
<div class="container-fluid calendar-admin-page p-0 px-2 py-2">
    @if(!isset($courseMaster) || $courseMaster->isEmpty())
    ...
    @endif
        {{-- OT timetable header (Time Table card + course info cards + Download) --}}
        <style>
            .tt-title-card {
                background: #fff;
            ...
            .tt-info-card .tt-info-label {
                font-size: 0.78rem;
                color: var(--text-muted, #6b7280);
                font-weight: 500;
            }
            .tt-info-card .tt-info-value {
                font-size: 0.98rem;
                font-weight: 700;
                color: #1f2937;
                line-height: 1.3;
            ...
            #calendar .fc-view-icon-btn .bi { font-size: 1.05rem; }
            ...
            /* ===== Minimal, flat calendar (match mockup) ===== */
            ...
            #calendar .fc-day-other .fc-daygrid-day-number { color: #c2c8d0; }
            ...
            #calendar .fc-event-card::before,
            #calendar .fc-event-card::after { display: none !important; }
            #calendar .fc-event-card:hover {
                transform: none !important;
                box-shadow: 0 2px 6px rgba(0, 0, 0, 0.08) !important;
                background: #fff !important;
                border-left-width: 4px !important;
            }
            #calendar .fc-event-card .event-title {
                font-size: 0.8rem !important;
            ...
            #calendar .fc-event-card .event-meta {
                border-top: none !important;
                margin-top: 0 !important;
                padding-top: 0 !important;
                gap: 0.25rem !important;
            }
            #calendar .fc-event-card .meta-item {
            ...
            #calendar .fc-event-card .meta-item--time {
                display: inline-flex !important;
            }
            #calendar .fc-event-card .event-badge { display: none !important; }

            /* Plain "+N More" text link (no blue pill) */
            #calendar .fc-daygrid-day-more-link,
            ...
            #calendar .fc-daygrid-day-more-link:hover,
            #calendar .fc-more-link:hover {
                background: transparent !important;
                text-decoration: underline !important;
                transform: none !important;
                color: #2d4aa7 !important;
                box-shadow: none !important;
            }
            ...
            .ot-hover-loading { color: #6b7280; font-size: 0.85rem; }
            .ot-hover-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
            .ot-hover-titles { min-width: 0; }
            .ot-hover-title { font-size: 1.15rem; font-weight: 700; color: #1f2937; line-height: 1.3; }
            .ot-hover-datetime { font-size: 0.85rem; color: #374151; margin-top: 2px; }
            .ot-hover-actions { display: flex; gap: 8px; flex-shrink: 0; }
            .ot-hover-btn {
            ...
            .ot-hover-box {
                background: #eef4fb; border-radius: 12px; padding: 12px 14px; margin-bottom: 12px;
            ...
            .ot-hover-card.tail-right .ot-hover-tail {
                right: -12px; border-left: 12px solid #fff;
                filter: drop-shadow(2px 0 1px rgba(0, 0, 0, 0.06));
            }
            .ot-hover-card.tail-left .ot-hover-tail {
                left: -12px; border-right: 12px solid #fff;
                filter: drop-shadow(-2px 0 1px rgba(0, 0, 0, 0.06));
            }

            @media print {
                .tt-title-card, .tt-info-card { box-shadow: none !important; }
                #downloadTimetableBtn, .ot-hover-card { display: none !important; }
            }
        </style>

        @php($otCourse = $courseMaster->first())

        <div class="tt-title-card shadow-sm p-3 p-md-4 mb-3">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-4">
                    <div class="tt-info-card">
                        <span class="tt-info-label">Course Name</span>
                        <span class="tt-info-value">{{ $otCourse->course_name ?? '—' }}</span>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="tt-info-card">
                        <span class="tt-info-label">Course Short Name/ Course Code</span>
                        <span class="tt-info-value">{{ $otCourse->couse_short_name ?? '—' }}</span>
                    </div>
                </div>
                <div class="col-12 col-md-4">
                    <div class="tt-info-card">
                        <span class="tt-info-label">Year</span>
                        <span class="tt-info-value">{{ $otCourse->course_year ?? date('Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-end mb-3">
            <button type="button" id="downloadTimetableBtn"
                class="btn d-inline-flex align-items-center gap-2 rounded-2 px-3 py-2">
        ...
        <script>
            document.getElementById('downloadTimetableBtn')?.addEventListener('click', function () {
                const base = "{{ route('calendar.ot.download') }}";
                const params = new URLSearchParams();

                // Export exactly what is currently visible in the calendar.
                const cm = window.calendarManager;
                const cal = cm && cm.calendar;
                if (cal && cal.view) {
                    const fmt = (d) => {
                        const x = new Date(d);
                    ...
                    params.append('start', fmt(cal.view.activeStart));
                    // activeEnd is exclusive in FullCalendar; step back one day.
                    const end = new Date(cal.view.activeEnd);
                    end.setDate(end.getDate() - 1);
                    params.append('end', fmt(end));
                }
                if (cm && cm.selectedCourseId) {
                    params.append('course_id', cm.selectedCourseId);
                }
...
const CalendarConfig = {
    api: {
        events: "{{ route('calendar.ot.event.calendar-details') }}",
        eventDetails: "{{ route('calendar.ot.event.Singlecalendar-details') }}",
...
class CalendarManager {
    ...
    initFullCalendar() {
        ...
        this.calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            // Re-apply icon toggles whenever the view/date range changes
            // (FullCalendar rebuilds the toolbar buttons and resets them to text).
            datesSet: () => this.iconifyViewButtons(),
    ...
    iconifyViewButtons() {
        // Turn the FullCalendar Month/Week toolbar buttons into icon toggles.
        const map = [
            { sel: '.fc-timeGridWeek-button', icon: 'bi-list-ul', label: 'List / week view' },
            { sel: '.fc-dayGridMonth-button', icon: 'bi-grid-fill', label: 'Month view' },
        ];
        map.forEach(({ sel, icon, label }) => {
            const btn = document.querySelector(sel);
            if (!btn) return;
            // Idempotent: re-apply on every call (FC resets buttons to text on view change).
            const desired = '<i class="bi ' + icon + '" aria-hidden="true"></i>';
            if (btn.innerHTML !== desired) btn.innerHTML = desired;
            btn.setAttribute('aria-label', label);
            btn.setAttribute('title', label);
            btn.classList.add('fc-view-icon-btn');
        });
    }

    styleMoreLinks() {
        // OT view keeps "+ more" as a plain text link (styled via CSS); skip the
        // inline blue-pill styling so the minimal mockup look is preserved.
        return;
    ...
    ensureHoverCard() {
        ...
        card.className = 'ot-hover-card';
        card.style.display = 'none';
        // Keep the card open while the pointer is over it (so edit/delete are reachable).
        card.addEventListener('mouseenter', () => { clearTimeout(this._hoverHideTimer); });
        card.addEventListener('mouseleave', () => this.hideHoverCard());
        document.body.appendChild(card);
        this.hoverCard = card;
        this.hoverCache = this.hoverCache || {};
        return card;
    }

    handleEventMouseEnter(info) {
        const id = info.event.id;
        // Skip holidays / non-timetable events.
        if (!id || String(id).startsWith('holiday_')) return;

        clearTimeout(this._hoverHideTimer);
        this._hoverTargetEl = info.el;
        const card = this.ensureHoverCard();

        const render = (data) => {
            if (this._hoverTargetEl !== info.el) return; // pointer already moved on
            card.innerHTML = this.buildHoverCardHtml(data);
            this.wireHoverActions(card, data.id);
            this.positionHoverCard(info.el);
        };

        if (this.hoverCache[id]) {
            render(this.hoverCache[id]);
            return;
        }
        // Lightweight loading state, then fetch.
        card.innerHTML = '<div class="ot-hover-loading">Loading…</div>';
        this.positionHoverCard(info.el);
        fetch(`${CalendarConfig.api.eventDetails}?id=${id}`, {
            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
        })
            .then(r => r.ok ? r.json() : Promise.reject(r))
            .then(data => { this.hoverCache[id] = data; render(data); })
            .catch(() => this.hideHoverCard());
    }

    handleEventMouseLeave() {
        // Delay so the pointer can travel from the event onto the card.
        this._hoverTargetEl = null;
        clearTimeout(this._hoverHideTimer);
        this._hoverHideTimer = setTimeout(() => this.hideHoverCard(), 180);
    }
    ...
    buildHoverCardHtml(data) {
        const esc = (s) => String(s ?? '').replace(/[&<>"']/g, c => (
            { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]
        ));

        // Date line: "5 May 2026 Monday 19:30 - 20:30"
        let dateLine = '';
        if (data.start) {
            ...
            if (!isNaN(d)) {
                ...
                dateLine = `${day} ${month} ${year} ${weekday}`;
            }
        }
        if (data.class_session) dateLine += (dateLine ? '  ' : '') + esc(data.class_session);

        const title = esc(data.title || data.topic || 'Event');
        const topic = data.topic ? esc(data.topic) : '';

        const canEdit = @json(hasRole('Training-Induction') || hasRole('Admin') || hasRole('Training-MCTP') || hasRole('IST'));
        const actions = canEdit ? `
            <div class="ot-hover-actions">
                <button type="button" class="ot-hover-btn ot-hover-del" data-act="delete" aria-label="Delete event"><i class="bi bi-trash"></i></button>
                <button type="button" class="ot-hover-btn ot-hover-edit" data-act="edit" aria-label="Edit event"><i class="bi bi-pencil"></i></button>
            </div>` : '';

        const facultyLine = data.faculty_name ? `<div><span class="lbl">Faculty:</span> ${esc(data.faculty_name)}</div>` : '';
        const groupLine = data.group_name ? `<div><span class="lbl">Group Name:</span> ${esc(data.group_name)}</div>` : '';
        const infoBox = (facultyLine || groupLine) ? `<div class="ot-hover-box">${facultyLine}${groupLine}</div>` : '';

        const venueBox = data.venue_name ? `
            <div class="ot-hover-box ot-hover-venue">
                <div><span class="lbl">Venue:</span> ${esc(data.venue_name)}</div>
                <i class="bi bi-geo-alt-fill" aria-hidden="true"></i>
            </div>` : '';

        return `
            <span class="ot-hover-tail" aria-hidden="true"></span>
            <div class="ot-hover-head">
                <div class="ot-hover-titles">
                    <div class="ot-hover-title">${title}</div>
                    ${dateLine ? `<div class="ot-hover-datetime">${dateLine}</div>` : ''}
                </div>
                ${actions}
            </div>
            <hr class="ot-hover-divider">
            ${topic ? `<div class="ot-hover-topic">${topic}</div>` : ''}
            ${infoBox}
            ${venueBox}
        `;
    }

    wireHoverActions(card, id) {
    ...
    positionHoverCard(targetEl) {
        ...
        if (spaceRight >= cw + gap) {
            left = rect.right + gap;      // card to the right, tail points left
            tailRight = false;
        } else {
            left = rect.left - cw - gap;  // card to the left, tail points right
            tailRight = true;
        }
        ...
        card.classList.toggle('tail-right', tailRight);
        card.classList.toggle('tail-left', !tailRight);
        // Vertical tail offset: align with the event's centre.
        const tailTop = Math.max(16, Math.min(ch - 24, rect.top + rect.height / 2 - top));
        card.style.setProperty('--tail-top', tailTop + 'px');
