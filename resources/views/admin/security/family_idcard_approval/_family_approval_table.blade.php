            ...
            <tbody>
                @forelse($groups as $group)
                    ...
                    <tr data-ts="{{ $ts }}">
                        ...
                        <td>
                            <span class="status-pill badge bg-{{ $group->phase_class ?? 'secondary' }}-subtle"
                        ...
                        <td class="text-center">
                            <div class="d-inline-flex align-items-center justify-content-center gap-3" role="group" aria-label="Row actions">
                                <a href="{{ route('admin.family_idcard.members', $group->first_id) }}{{ $familyMembersQueryString }}"
                                   class="fia-act fia-act--view" title="View Members">
                                ...
                                @if($group->can_approve ?? false)
                                    <form action="{{ route('admin.security.family_idcard_approval.approve_group', encrypt($group->first_id)) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button type="submit" class="fia-act fia-act--approve" title="Approve"
                                                onclick="return confirm('Are you sure you want to approve?')">
                                            <i class="bi bi-check-circle" aria-hidden="true"></i><span>Approve</span>
                                        </button>
                                    </form>
                                    <button type="button" class="fia-act fia-act--reject" title="Reject"
