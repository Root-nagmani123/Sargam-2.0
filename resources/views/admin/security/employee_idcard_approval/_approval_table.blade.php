                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex gap-1 flex-wrap">
                            <form action="{{ $approvalStage === 1 ? route('admin.security.employee_idcard_approval.approve1', encrypt($req->id)) : route('admin.security.employee_idcard_approval.approve2', encrypt($req->id)) }}" method="POST" class="d-inline">
                                @csrf
                            ...
                            <button type="button" class="btn btn-link p-0 text-danger text-decoration-none reject-btn" title="Reject"
                                data-name="{{ $req->name }}"
                                data-url="{{ $approvalStage === 1 ? route('admin.security.employee_idcard_approval.reject1', encrypt($req->id)) : route('admin.security.employee_idcard_approval.reject2', encrypt($req->id)) }}">Reject</button>
                        </div>
                    </td>
