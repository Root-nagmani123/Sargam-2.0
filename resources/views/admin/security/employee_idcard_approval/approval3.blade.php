...
.a3-status-tabs .programme-status-pill.active .badge.bg-white { background-color: #fff !important; color: #004a93 !important; }
.a3-status-tabs .programme-status-pill .badge { font-weight: 600; }
</style>
@endpush

@section('content')
@php
    $a3ActiveTab = $activeTab ?? 'new';
    $a3DateFrom = request('date_from', '');
    $a3DateTo = request('date_to', '');
@endphp

<div class="container-fluid idcard-approval3-page py-3">
    <x-breadcrum title="Approval III — Employee ID Card">
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('admin.security.employee_idcard_approval.approval2') }}"
    ...
    <ul class="nav nav-pills gap-2 p-1 rounded-1 programme-status-tabs a3-status-tabs bg-white mb-3 flex-wrap"
        id="approval3Tabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link rounded-1 px-3 py-2 fw-semibold programme-status-pill {{ $a3ActiveTab === 'new' ? 'active' : '' }}"
                    id="new-request-tab3" data-bs-toggle="tab" data-bs-target="#new-request-panel3" type="button" role="tab"
                    aria-controls="new-request-panel3" aria-selected="{{ $a3ActiveTab === 'new' ? 'true' : 'false' }}" data-tab-key="new">
                New Request
                <span class="badge rounded-1 bg-white text-primary ms-1">{{ $newRequests->count() }}</span>
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link rounded-1 px-3 py-2 fw-semibold programme-status-pill {{ $a3ActiveTab === 'for_approval' ? 'active' : '' }}"
                    id="for-approval-tab3" data-bs-toggle="tab" data-bs-target="#for-approval-panel3" type="button" role="tab"
                    aria-controls="for-approval-panel3" aria-selected="{{ $a3ActiveTab === 'for_approval' ? 'true' : 'false' }}" data-tab-key="for_approval">
                Processed Request
                <span class="badge rounded-1 bg-secondary ms-1">{{ $forApprovalRequests->count() }}</span>
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link rounded-1 px-3 py-2 fw-semibold programme-status-pill {{ $a3ActiveTab === 'issued' ? 'active' : '' }}"
                    id="issued-tab3" data-bs-toggle="tab" data-bs-target="#issued-panel3" type="button" role="tab"
                    aria-controls="issued-panel3" aria-selected="{{ $a3ActiveTab === 'issued' ? 'true' : 'false' }}" data-tab-key="issued">
                Issued
                <span class="badge rounded-1 bg-success ms-1">{{ $issuedRequests->count() }}</span>
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link rounded-1 px-3 py-2 fw-semibold programme-status-pill {{ $a3ActiveTab === 'rejected' ? 'active' : '' }}"
                    id="rejected-tab3" data-bs-toggle="tab" data-bs-target="#rejected-panel3" type="button" role="tab"
                    aria-controls="rejected-panel3" aria-selected="{{ $a3ActiveTab === 'rejected' ? 'true' : 'false' }}" data-tab-key="rejected">
                Rejected
        ...
        <div class="card-body p-3 p-md-4">
            ...
            <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-4 programme-dt-toolbar">
                <div class="d-flex flex-wrap align-items-center gap-3">
                    ...
                    {{-- Request Date range reloads the page: the server bounds the result
                         set by date, so filtering only in the browser would hide records
                         outside the loaded range. --}}
                    <form method="GET" action="{{ route('admin.security.employee_idcard_approval.approval3') }}" id="filterForm3" class="d-inline">
                        <input type="hidden" name="tab" id="activeTabInput3" value="{{ $a3ActiveTab }}">
                        <input type="hidden" name="date_from" id="a3DateFrom" value="{{ $a3DateFrom }}">
                        <input type="hidden" name="date_to" id="a3DateTo" value="{{ $a3DateTo }}">
                    </form>

                    <div class="dropdown">
                        <button type="button" class="idcp-toggle dropdown-toggle"
                                id="a3TimePeriodToggle" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false">
                            <i class="material-icons material-symbols-rounded" style="font-size:18px;" aria-hidden="true">calendar_month</i>
                            <span id="a3TimePeriodLabel">Time Period</span>
                        </button>
                        <div class="dropdown-menu p-0 idcp-menu">
                            <div class="idcp-cal" id="a3Calendar">
                                ...
                                <div class="idcp-cal-footer">
                                    <span class="idcp-cal-range" id="a3CalRange">Select a date range</span>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-sm btn-outline-secondary" id="a3ClearPeriod">Clear</button>
                                        <button type="button" class="btn btn-sm btn-primary" id="a3ApplyPeriod">Apply</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <a href="{{ route('admin.security.employee_idcard_approval.approval3') }}" class="btn programme-dt-btn-reset">Reset Filters</a>
                </div>

                <div class="d-flex flex-wrap align-items-center gap-2 ms-lg-auto">
                    <button type="button" class="btn programme-dt-btn-columns" id="a3BtnColumns"
                        data-bs-toggle="modal" data-bs-target="#a3ColumnVisibilityModal" title="Show / hide columns">
                        <span>Columns</span><i class="bi bi-layout-three-columns" aria-hidden="true"></i>
                    </button>
                    {{-- One search slot per table; only the active tab's is visible. --}}
                    <div class="programme-dt-search a3-search" data-tab-key="new" data-dt-search-for="a3TableNew"></div>
                    <div class="programme-dt-search a3-search d-none" data-tab-key="for_approval" data-dt-search-for="a3TableFor"></div>
                    <div class="programme-dt-search a3-search d-none" data-tab-key="issued" data-dt-search-for="a3TableIssued"></div>
                    <div class="programme-dt-search a3-search d-none" data-tab-key="rejected" data-dt-search-for="a3TableRejected"></div>
                </div>
            </div>

            <div class="tab-content">
                <div class="tab-pane {{ $a3ActiveTab === 'new' ? 'show active' : '' }}" id="new-request-panel3" role="tabpanel" aria-labelledby="new-request-tab3">
                    @include('admin.security.employee_idcard_approval._approval_table', [
                        'requests' => $newRequests, 'approvalStage' => 3, 'tableId' => 'a3TableNew',
                    ])
                </div>
                <div class="tab-pane {{ $a3ActiveTab === 'for_approval' ? 'show active' : '' }}" id="for-approval-panel3" role="tabpanel" aria-labelledby="for-approval-tab3">
                    @include('admin.security.employee_idcard_approval._approval_table', [
                        'requests' => $forApprovalRequests, 'approvalStage' => 3, 'tableId' => 'a3TableFor',
                    ])
                </div>
                <div class="tab-pane {{ $a3ActiveTab === 'issued' ? 'show active' : '' }}" id="issued-panel3" role="tabpanel" aria-labelledby="issued-tab3">
                    @include('admin.security.employee_idcard_approval._approval_table', [
                        'requests' => $issuedRequests, 'approvalStage' => 3, 'tableId' => 'a3TableIssued',
                    ])
                </div>
                <div class="tab-pane {{ $a3ActiveTab === 'rejected' ? 'show active' : '' }}" id="rejected-panel3" role="tabpanel" aria-labelledby="rejected-tab3">
                    @include('admin.security.employee_idcard_approval._approval_table', [
                        'requests' => $rejectedRequests, 'approvalStage' => 3, 'tableId' => 'a3TableRejected',
                    ])
                </div>
            </div>
        </div>
...
<div class="modal fade" id="a3ColumnVisibilityModal" tabindex="-1" aria-labelledby="a3ColumnVisibilityLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-0 pb-2">
                <h5 class="modal-title fw-bold" id="a3ColumnVisibilityLabel">Column Visibility</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body pt-0">
                <hr class="mt-0">
                <div class="row g-3" id="a3ColumnToggleGrid"></div>
            </div>
            <div class="modal-footer border-0">
...
$(function () {
    ...
    var TABLES = {
        new:          { id: 'a3TableNew',      dt: null },
        for_approval: { id: 'a3TableFor',      dt: null },
        issued:       { id: 'a3TableIssued',   dt: null },
        rejected:     { id: 'a3TableRejected', dt: null }
    };
    var TAB_KEYS = ['new', 'for_approval', 'issued', 'rejected'];
    var currentTab = @json($a3ActiveTab);

    function initTable(tableId) {
        var $table = $('#' + tableId);
        if (!$table.length) { return null; }
        // Skip the empty-state row so its message stays visible.
    ...
    function updateTimePeriodLabel() {
        var from = $('#a3DateFrom').val();
        var to = $('#a3DateTo').val();
        $('#a3TimePeriodLabel').text((from || to) ? ((from || '…') + ' → ' + (to || '…')) : 'Time Period');
    }
    ...
    (function initRangeCalendar() {
        ...
        function render(){
            ...
            $('#a3Calendar .idcp-cal-month[data-month="0"]').html(buildMonth(left));
            $('#a3Calendar .idcp-cal-month[data-month="1"]').html(buildMonth(right));
            $('#a3Calendar .idcp-cal-month[data-month="0"] [data-nav="next"]').css('visibility', 'hidden');
            $('#a3Calendar .idcp-cal-month[data-month="1"] [data-nav="prev"]').css('visibility', 'hidden');
            var label = 'Select a date range';
            if (startD && endD) label = ymd(startD) + '  →  ' + ymd(endD);
            else if (startD) label = ymd(startD) + '  → …';
            $('#a3CalRange').text(label);
        }

        $('#a3Calendar').on('click', '.idcp-cal-nav', function(){
            var dir = $(this).data('nav') === 'prev' ? -1 : 1;
            view = new Date(view.getFullYear(), view.getMonth() + dir, 1);
            render();
        });
        $('#a3Calendar').on('click', '.idcp-cal-day', function(){
        ...
        $('#a3ApplyPeriod').on('click', function(){
            $('#a3DateFrom').val(startD ? ymd(startD) : '');
            $('#a3DateTo').val(endD ? ymd(endD) : (startD ? ymd(startD) : ''));
            $('#activeTabInput3').val(currentTab);
            document.getElementById('filterForm3').submit();
        });
        $('#a3ClearPeriod').on('click', function(){
            startD = null; endD = null; render();
            $('#a3DateFrom').val(''); $('#a3DateTo').val('');
            $('#activeTabInput3').val(currentTab);
            document.getElementById('filterForm3').submit();
        });

        startD = parseYmd($('#a3DateFrom').val());
        endD = parseYmd($('#a3DateTo').val());
    ...
    var colKeyPrefix = 'a3ApprovalGrid:hiddenColumns:v1:';
    ...
    $('#a3ColumnVisibilityModal').on('show.bs.modal', buildColumnsModal);

    /* ---- Tab change: track active tab, swap search box, fix widths, sync URL ---- */
    $('#approval3Tabs .nav-link').on('shown.bs.tab', function () {
        currentTab = this.dataset.tabKey || 'new';
        $('#activeTabInput3').val(currentTab);
        $('.a3-search').each(function () {
