...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const table = $('#designationMasterTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: { url: "{{ route('admin.setup.designation_master.index') }}", type: 'GET' },
        columns: [
    ...
    document.addEventListener('click', e => {
        const link = e.target.closest('.openEditDesignation');
        if (!link) return;
        e.preventDefault();
        loadForm(link.getAttribute('href'), 'Edit Designation');
    });
