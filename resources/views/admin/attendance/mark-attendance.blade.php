...
@push('styles')
<link rel="stylesheet"
    href="{{ asset('css/mark-attendance-admin.css') }}?v={{ @filemtime(public_path('css/mark-attendance-admin.css')) ?: time() }}">
@endpush
    ...
    <div class="container-fluid mark-att-master-page">
        ...
        <div class="d-flex justify-content-end mb-2 gap-2">
            <a href="{{ route('attendance.export', ['group_pk' => $group_pk, 'course_pk' => $course_pk, 'timetable_pk' => $courseGroup->timetable_pk]) }}"
                class="btn btn-outline-primary d-inline-flex align-items-center justify-content-center fw-semibold shadow-sm text-nowrap mark-att-btn-download border-0">
                <i class="bi bi-download flex-shrink-0 me-2 " aria-hidden="true"></i>
                <span>Download</span>
            </a>
        </div>
        <x-session_message />
        <div class="card mark-att-dt-card border-0 shadow-sm rounded-3 overflow-hidden">
            <div class="card-body p-3 p-md-4">
                <div
                    class="d-flex flex-column flex-xl-row align-items-xl-center justify-content-between gap-3 mb-4 programme-dt-toolbar mark-att-dt-toolbar w-100">
                    <div class="d-flex flex-wrap align-items-center gap-3">
                        ...
                        <div class="programme-dt-filter-select mark-att-course-filter">
                            ...
                            <select id="mark_att_course_display" class="form-select form-select-sm" disabled
                        ...
                        <div class="programme-dt-filter-select mark-att-period-filter">
                            ...
                            <select id="mark_att_period_display" class="form-select form-select-sm" disabled
                    ...
                    <div class="mark-att-table-search-slot ms-xl-auto flex-shrink-0">
                        <div class="dropdown">
                            <button type="button" class="btn mark-att-search-trigger" id="markAttSearchTrigger"
                                data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false"
                                aria-label="Search table">
                                <i class="bi bi-search" aria-hidden="true"></i>
                            </button>
                            <div
                                class="dropdown-menu dropdown-menu-end shadow-sm border-0 rounded-3 p-3 mark-att-search-menu">
                                <label for="markAttTableSearch"
                                    class="form-label small text-secondary mb-2">Search</label>
                                <div id="markAttDtSearch" class="mark-att-dt-search-host"
                                    data-dt-search-for="studentAttendanceTable"></div>
                            </div>
                        </div>
                    </div>
                </div>
