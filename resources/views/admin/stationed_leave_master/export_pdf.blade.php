    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th class="col-approval">Approval Required</th>
                <th class="col-count">Faculty Count</th>
        ...
        <tbody>
            @forelse($rows as $index => $row)
                ...
                <tr>
                    ...
                    <td class="col-date">{{ $row->effective_from?->format('d-m-Y') ?? 'N/A' }}</td>
                    <td class="col-timing">{{ blank($cutoffTime) ? 'N/A' : \Carbon\Carbon::parse($cutoffTime)->format('h:i A') }}</td>
                    <td class="col-approval">{{ (int) $row->is_faculty_approval_required === 1 ? 'Yes' : 'No' }}</td>
                    <td class="col-count">{{ (int) ($row->approvers_count ?? 0) }}</td>
                    <td class="col-status"><span class="badge badge-{{ $isActive ? 'active' : 'inactive' }}">{{ $isActive ? 'Active' : 'Inactive' }}</span></td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" style="text-align:center;">No stationed leave configuration found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
