@php
    ...
    $defs = EstateHacApprovedExport::columnDefs();
...
<head>
    ...
    <title>HAC Approval - LBSNAA MUSSOORIE</title>
    <style>
...
<body>
    ...
    <div class="report-title-block">
        <h1 class="report-title">HAC Approval</h1>
    ...
    <table class="data-table">
        ...
        <tbody>
                ...
                <tr>
                    @foreach($columns as $column)
                        ...
                        <td class="{{ $column['center'] ? 'text-center' : '' }}">
                            @if($column['key'] === 'request_type')
                                <span class="req-type req-type--{{ $value === 'Change Request' ? 'change' : 'new' }}">{{ $value }}</span>
                            @else
            ...
            @empty
                <tr class="empty-row">
                    <td colspan="{{ $colCount }}">No HAC requests found for the applied filters.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
