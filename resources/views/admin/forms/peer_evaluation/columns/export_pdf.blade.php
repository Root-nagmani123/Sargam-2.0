{{-- Manage Evaluation Columns → PDF (DomPDF).
...
<head>
    <meta charset="utf-8">
    <title>Manage Evaluation Columns — LBSNAA</title>
    <style>
        ...
        * { font-family: 'DejaVu Sans', sans-serif; }
        ...
        .col-name    { width: 16%; }
        .col-type    { width: 11%; text-align: center; }
...
<body>
    ...
    <div class="report-title">MANAGE EVALUATION COLUMNS</div>
    <div class="meta">
        @if(filled($filterText)){{ $filterText }} &nbsp;|&nbsp; @endif Generated: {{ $exportDate }}
    </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            @empty
                <tr><td colspan="{{ count($columns) }}" class="empty">No columns to export</td></tr>
            @endforelse
        </tbody>
    </table>
    ...
    {{-- Page numbers on every page. Must be the LAST thing in <body>: DomPDF only
         resolves the page count once the whole document is laid out, so a script
         placed earlier renders every page as "Page N of 1". --}}
