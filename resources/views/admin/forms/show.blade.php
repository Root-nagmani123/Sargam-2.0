    ...
    <div class="container">
        <div class="sidebar">
            ...
            @if($form)
                ...
                {{-- <a href="{{ route('forms.download', ['formId' => $form->id, 'userId' => auth()->id()]) }}" class="btn" style="text-align: left;"> --}}
                    {{-- Download PDF --}}
                </a>
            @else
                                    ...
                                    <tr>
                                        @for($i = 0; $i <= $maxCol; $i++)
                                            @if(isset($headersBySection[$section->id][$i]))
                                                ...
                                                <input type="hidden" name="header_{{ $section->id }}_{{ $i }}" value="{{ $headersBySection[$section->id][$i] }}">
                                            @else
                                                <th></th>
                                            @endif
                                        @endfor
                                    ...
                                    <tr>
                                        <td colspan="{{ $maxCol + 1 }}" style="text-align: left;">
                                            <button class="replicate-row btn btn-success" onclick="replicateRow(event)" style="cursor: pointer; border: none; background: none; padding: 0;">
                                                <img src="{{ asset('images/increase.png') }}" alt="Increase" style="width: 15px; height: 15px;">
                                            </button>
                                            ...
                                            <button class="remove-row btn btn-danger" onclick="removeRow(event)" style="cursor: pointer; border: none; background: none; padding: 0;">
                                                <img src="{{ asset('images/decrease.png') }}" alt="Decrease" style="width: 15px; height: 15px;">
                                            </button>
                                        </td>
                                    </tr>
                                </tfoot>
