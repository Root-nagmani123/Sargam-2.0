{{-- Print export for a role's dashboard-card assignment.

     A server-rendered page, not window.print() — the same controller action
     serves this and the CSV off one row set and one column list, so the
     printout and the download can't drift apart
     (docs/new-design-index-page.md §1). --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>Dashboard Cards — {{ $role->name }} — LBSNAA</title>
    <style>
        ...
        .rp-print-sno     { width: 7%;  text-align: center; }
        .rp-print-name    { width: 30%; }
        .rp-print-icon    { width: 20%; }
        .rp-print-colour  { width: 12%; }
        .rp-print-order   { width: 8%;  text-align: center; }
        .rp-print-created { width: 12%; text-align: center; }
        .rp-print-status  { width: 11%; text-align: center; }
    </style>
</head>
<body onload="window.print();">
    ...
    <div class="rp-print-title">Dashboard Cards — {{ $role->name }}</div>
    ...
    {{-- Columns come from RoleService::dashboardExportColumnDefs(), already
    ...
    <table class="rp-print-table">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="{{ max(count($columns), 1) }}" class="rp-print-empty">No dashboard cards to print</td>
                </tr>
            @endforelse
        </tbody>
    </table>
