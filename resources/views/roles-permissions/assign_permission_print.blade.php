{{-- Print export for a role's permission matrix.

     A server-rendered page, not window.print() — the same controller action
     serves this and the CSV off one matrix and one column list, so the printout
     and the download can't drift apart (docs/new-design-index-page.md §1).

     Landscape: 7 columns of identifier-length text will not fit A4 portrait. --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>Permissions — {{ $role->name }} — LBSNAA</title>
    <style>
        ...
        table.rp-print-table { width: 100%; border-collapse: collapse; font-size: 8.5px; table-layout: fixed; }
        ...
        .rp-print-sno        { width: 5%;  text-align: center; }
        .rp-print-category   { width: 12%; }
        .rp-print-group      { width: 16%; }
        .rp-print-menu       { width: 17%; }
        .rp-print-submenu    { width: 17%; }
        .rp-print-permission { width: 23%; }
        .rp-print-status     { width: 10%; text-align: center; }
    </style>
</head>
<body onload="window.print();">
    ...
    <div class="rp-print-title">Permissions — {{ $role->name }}</div>
    <div class="rp-print-meta">Generated: {{ $exportDate }}</div>

    @if (filled($filterLine))
        <div class="rp-print-filters">{!! $filterLine !!}</div>
    @endif

    <div class="rp-print-total">Total Records: {{ number_format(count($rows)) }}</div>

    {{-- Columns come from RoleService::permissionExportColumnDefs(), already
    ...
    <table class="rp-print-table">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="{{ max(count($columns), 1) }}" class="rp-print-empty">No permissions to print</td>
                </tr>
            @endforelse
        </tbody>
    </table>
