            ...
            <form action="{{ route('admin.mess.material-management.store') }}" method="POST">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="inve_item_master_pk" class="form-select" required>
                            ...
                            @foreach($items as $item)
                                <option value="{{ $item->id }}" {{ old('inve_item_master_pk') == $item->id ? 'selected' : '' }}>
                                    {{ $item->item_name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="requested_store_id" class="form-select">
                            ...
                            @foreach($stores as $store)
                                <option value="{{ $store->id }}" {{ old('requested_store_id') == $store->id ? 'selected' : '' }}>
                                    {{ $store->store_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="payment_type" class="form-select" required>
                            <option value="0" {{ old('payment_type') == '0' ? 'selected' : '' }}>Unpaid</option>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="transfer_to" class="form-control" value="{{ old('transfer_to') }}">
                    </div>
