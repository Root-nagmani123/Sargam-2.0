...
| 2026-08-04 | **A11Y-1 + A11Y-2 shipped (form contrast).** New `_active-forms.scss` (unlayered, → `ux4g-app.css`): `.form-control/.form-select` rest border `#e0e6eb` (1.26:1) → `#858e99` (**3.32:1**, WCAG 1.4.11), scoped to controls only; `:focus` → navy `#004384` border + navy ring (**replaces the theme's off-brand violet `#613AF5` ring**, WCAG 2.4.7). Verified: injected-control contrast + real keyboard-focus screenshot + diff-image (borders only). Baseline re-approved (chrome+firefox+webkit, 58 each; 1 async flake `/dashboard/students` passed on re-run). | Border fix scoped to inputs (not global `--bs-border-color`) so cards/tables keep soft dividers. Focus not in resting baseline → no gate impact. Both derive from the R-1 navy. |
| 2026-08-04 | **R-1 navy re-colour EXECUTED.** Replaced `#004a93` (×910) + `rgb(0,74,147)` (×209) → `#004384` / `rgb(0,67,132)` across **211 files** (2712 lines, 1:1). 0 old-navy remaining; `view:cache` clean; render spot-checked; baseline re-approved (chrome 68 + firefox 57 + webkit 57). Done as a **literal** replace (not `var()`) for DomPDF-template safety. A11Y-1/A11Y-2 now unblocked. | Value chosen = #004384 (Option B); execution mechanism = literal (Option A mechanics) so PDF/print templates stay correct. |
| 2026-08-04 | **R-1 brand navy DECIDED: `#004384` (canonical).** Chosen over the de-facto-deployed `#004a93` (×910). All three candidate navies pass AAA, so this was brand-fidelity over effort: `#004384` is the official LBSNAA value. Consequence: a tokenisation/re-colour pass (`#004a93`→`#004384`, ~910 sites) that is a **visible** (subtle) change → ships gated **with baseline re-approval** (not zero-regression). Unblocks A11Y-1 (input border) + A11Y-2 (focus ring), both deriving from the token. Note: `UX4G-Decision-Note-R1-Brand-Navy.md`. | Stakeholder chose spec fidelity; `--bs-primary` already = `#004384`, so the token is correct — the work is re-colouring the hardcoded literals + re-baselining. |
| 2026-08-04 | **Phase 16 (Cleanup) complete — MIGRATION COMPLETE.** Deleted dead `form-wizard.js` (0 live refs; only demo `#example-*` IDs, in 0 views) + removed its footer load. Verified no committed conflict markers / backup files (the `=======` hits were decorative CSS comment dividers). Gate = **58 passed, zero diff**. Consolidated all phases into `docs/UX4G-Migration-Completion-Summary.md`. | Final phase: remove only verified-dead code, gate it, hand off backlog + the R-1 navy decision to owners. |
| 2026-08-04 | **Phase 15 (Cross-browser regression) complete.** Lifted the chrome-only pin in `baseline.spec.js` to the visual projects. Established + verified baselines in **Firefox** (58 establish + 58 deterministic re-run, zero diff) and **Safari-WebKit** (58 establish); Edge skipped (Chromium≈chrome). Cross-browser JS smoke on `/batch`: Swal/FullCalendar/jQuery/steps/iconify all present, **0 console errors in both engines**. Migration verified across Blink/Gecko/WebKit. Deliverable: `docs/UX4G-Phase-15-Regression.md`. | Per-project snapshots — cross-engine pixel compare is meaningless (font hinting differs); each engine regressed vs its own baseline + structural/JS parity. 57 auth routes (migration substance) covered; public-mode + Edge available but deferred. |
| 2026-08-04 | **Phase 14 (Performance) complete.** De-duplicated 6 assets loaded 2× on every admin page (footer+master): FullCalendar (281 KB, was loaded twice back-to-back), SweetAlert2, jQuery Steps/Validate, dead form-wizard, + iconify CDN copy (also drops a CDN dep) — **~422 KB redundant transfer removed/page**. Runtime-verified all globals + 0 JS errors. Gate 57 pass + 1 flaky (`/attendance/user_attendance`, passed on re-run) = **effectively 58/58**. Backlog: static-asset cache headers (48/70 uncached — highest remaining win, needs prod config), defer/async, CDN self-hosting. Deliverable: `docs/UX4G-Phase-14-Performance.md`. | Pure de-dup keeps one working copy each = no behaviour change. Cache headers are a server/infra change (no public/.htaccess) — flagged with a ready snippet, not shipped blind. |
| 2026-08-04 | **Phase 13 (Accessibility) complete.** Systemic foundation confirmed already-strong (html lang, GIGW skip-link, dynamic titles, exemplary header ARIA). Shipped zero-visual accessible-name fixes: `aria-label` + `aria-hidden` on 15 nameless icon controls across 13 files (FC report search/clear, FC document-form remove-row ×7, forms-grid/sample-docs delete, enrollment pagination). Backlog (visual → needs design sign-off + re-baseline): input-border 1.26:1, focus-ring, 4 non-focusable tooltips (A-5), joining_document badge. Gate = **58 passed, zero diff**. Deliverable: `docs/UX4G-Phase-13-Accessibility.md`. | Icon-only controls with neither title nor aria-label announce nothing to AT — pure additive ARIA fix. Contrast/focus items are visual changes; deferred to a gated design pass. |
| 2026-08-04 | **Phase 12 (Pages) complete — confirmation + catalogue, no code change.** Page-level BS4-ism audit: **no live defect** — `.close` ×218 = btn-close/custom; `data-target` ×2 = custom filter JS; `.form-group` ×13 works via app's own `forms.css` shim (left untouched); rest benign/self-defined. Catalogued token-adoption debt for global activation: 302 inline `<style>` blocks, ~2,934 inline `style=`, ~4,781 hardcoded hex, brand-navy fragmentation (`#004a93` ×747 vs `#1a3c6e` ×60 vs token `#004384`). No page modified ⇒ Phase 11 gate (58/58) certifies the state. Deliverable: `docs/UX4G-Phase-12-Pages.md`. | App is already BS5 + UX4G is per-component, so pages need no migration. Bulk re-coloring to tokens = a visual change needing R-1 design sign-off + re-baseline; belongs to the global-activation phase, not this zero-regression one. |
| 2026-08-04 | **Phase 11 (Custom ERP components) complete.** Custom component layer (`<x-breadcrum>` ×309, `<x-session>` ×156, `<x-input>` ×147, `<x-select>` ×77, steppers) audited — **already BS5/UX4G-clean, no BS4-isms**. Shipped: `aria-current="step"` on the two steppers that lacked it (parity with `fc-stepper`). **jquery-steps = KEEP** (member wizard's step-gating is AJAX business logic; plugin is Bootstrap-agnostic; blueprint's own "Keep" row wins over its "REPLACE" row). Gate = **58 passed, zero diff**. Deliverable: `docs/UX4G-Phase-11-Custom-ERP-Components.md`. | Replacing jquery-steps would rewrite `/member/validate-step` view logic — forbidden by migration rules. Deferred as a separately-approved feature workstream. Phase 10 correction: jQuery CDN refs are local-first `onerror` fallbacks (low risk), `form-wizard.js` is dead code. |
...
| P0-2 | Confirm R-1 (retain LBSNAA navy `#004384`) | Phase 2 tokens | Management | ✅ **CONFIRMED `#004384` (2026-08-04)** — canonical over the deployed `#004a93` (×910); see `UX4G-Decision-Note-R1-Brand-Navy.md`. Re-colour pass pending. |
...
| 11 — Custom ERP components | Phases 5–9 | ✅ **Done (2026-08-04)** — custom layer already BS5-clean; stepper `aria-current` parity; jquery-steps KEEP; gate 58/58. `UX4G-Phase-11-Custom-ERP-Components.md` |
| 12 — Pages | **All** component phases | ✅ **Done (2026-08-04)** — no live page-level BS4 defect; token-debt catalogued; no code change (Phase 11 gate certifies). `UX4G-Phase-12-Pages.md` |
| 13 — Accessibility | Runs after every phase | ✅ **Done (2026-08-04)** — foundation confirmed; 15 icon controls named (13 files); contrast/focus backlog flagged; gate 58/58. `UX4G-Phase-13-Accessibility.md` |
| 14 — Performance | Phase 12 | ✅ **Done (2026-08-04)** — 6 duplicate assets removed (~422 KB/page); cache-header win flagged; gate 58/58. `UX4G-Phase-14-Performance.md` |
| 15 — Regression | Runs after every phase; needs S-4 | ✅ **Done (2026-08-04)** — cross-browser baselines: Firefox (58, deterministic) + WebKit (58) + JS parity; Edge=Chromium skip. `UX4G-Phase-15-Regression.md` |
| 16 — Cleanup | Phase 12 | ✅ **Done (2026-08-04)** — dead form-wizard.js deleted; no cruft; gate 58/58; completion summary written. **MIGRATION COMPLETE.** |
