...
/**
 * Caste Master: the modernised listing, its four exports, and the Add/Edit
 ...
 */
class CasteCategoryMasterTest extends TestCase
{
    ...
    {
        $html = $this->fetch('/master/caste-category')->assertOk()->getContent();

        foreach ([
            'caste-page',
            'programme-dt-table',
            'programme-dt-panel',
            'data-dt-search-for="castecategorymaster-table"',
            'data-dt-footer-for="castecategorymaster-table"',
            'cstColumnToggleGrid',
            'cstFormModal',
            'cstDownloadBtn',
            'cstPrintBtn',
    ...
    {
        $columns = [];
        foreach (['DT_RowIndex', 'Seat_name', 'Seat_name_hindi', 'status', 'action'] as $i => $name) {
            $columns[$i] = [
                'data' => $name, 'name' => $name,
                'searchable' => in_array($name, ['Seat_name', 'Seat_name_hindi'], true) ? 'true' : 'false',
        ...
        if (! $row) {
            $this->markTestSkipped('no caste categories in this database');
        }
        ...
        $this->assertStringContainsString('cst-edit-btn', $row['action']);
        $this->assertStringContainsString('data-table="caste_category_master"', $row['action']);
    ...
    {
        $csv = $this->bytes($this->fetch('/master/caste-category/export/csv')->assertOk());
        $lines = str_getcsv($csv, "\n");

        $this->assertStringContainsString('CASTE MASTER', $lines[1]);
        $this->assertSame(['S. No.', 'Category/Caste Name', 'Category/Caste Name (Hindi)', 'Status'], str_getcsv($lines[5]));

        $print = $this->fetch('/master/caste-category/export/print')->assertOk()->getContent();
        $this->assertStringContainsString('window.print()', $print);
        $this->assertStringContainsString('>Category/Caste Name<', $print);

        $this->assertStringStartsWith('%PDF-', $this->bytes($this->fetch('/master/caste-category/export/pdf')->assertOk()));
        $this->assertStringStartsWith('PK', $this->bytes($this->fetch('/master/caste-category/export/excel')->assertOk()));

        $narrowed = $this->bytes($this->fetch('/master/caste-category/export/csv?cols=sno,seat_name,status'));
        $this->assertSame(['S. No.', 'Category/Caste Name', 'Status'], str_getcsv(str_getcsv($narrowed, "\n")[5]));
    ...
    {
        $this->fetch('/master/caste-category/export/exe')->assertNotFound();
    }
    ...
    {
        $user = $this->actor();
        $before = CasteCategoryMaster::count();
        ...
        try {
            ...
            $row = CasteCategoryMaster::where('Seat_name', $name)->first();
            ...
            $this->assertSame($renamed, $row->fresh()->Seat_name);

            // Both columns are varchar(30): longer names must be rejected, not truncated.
            $this->actingAs($user)
                ->withHeaders(['X-Requested-With' => 'XMLHttpRequest', 'Accept' => 'application/json'])
                ->postJson('/master/caste-category/store', ['pk' => '', 'Seat_name' => str_repeat('x', 31), 'Seat_name_hindi' => str_repeat('y', 31)])
                ->assertStatus(422)
                ->assertJsonValidationErrors('Seat_name');
        ...
        $this->assertSame($before, CasteCategoryMaster::count(), 'the test left rows behind');
    }
}
