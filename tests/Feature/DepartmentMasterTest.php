...
/**
 * Department Master: the modernised listing, its four exports, and the Add/Edit
 ...
 */
class DepartmentMasterTest extends TestCase
{
    ...
    {
        $html = $this->fetch('/master/department-master')->assertOk()->getContent();

        foreach ([
            'department-page',
            'programme-dt-table',
            'programme-dt-panel',
            'data-dt-search-for="departmentmaster-table"',
            'data-dt-footer-for="departmentmaster-table"',
            'dpmColumnToggleGrid',
            'dpmFormModal',
            'dpmDownloadBtn',
            'dpmPrintBtn',
    ...
    {
        $columns = [];
        foreach (['DT_RowIndex', 'department_name', 'status', 'action'] as $i => $name) {
            $columns[$i] = [
                'data' => $name, 'name' => $name,
                'searchable' => $name === 'department_name' ? 'true' : 'false',
        ...
        $json = $this->actingAs($this->actor())
            ...
            ->getJson('/master/department-master?' . http_build_query([
        ...
        if (! $row) {
            $this->markTestSkipped('no departments in this database');
        }
        ...
        $this->assertStringContainsString('dpm-edit-btn', $row['action']);
        $this->assertStringContainsString('data-table="department_master"', $row['action']);
    ...
    {
        $csv = $this->bytes($this->fetch('/master/department-master/export/csv')->assertOk());
        $lines = str_getcsv($csv, "\n");

        $this->assertStringContainsString('DEPARTMENT MASTER', $lines[1]);
        $this->assertSame(['S. No.', 'Department Name', 'Status'], str_getcsv($lines[5]));

        $print = $this->fetch('/master/department-master/export/print')->assertOk()->getContent();
        $this->assertStringContainsString('window.print()', $print);
        $this->assertStringContainsString('>Department Name<', $print);

        $this->assertStringStartsWith('%PDF-', $this->bytes($this->fetch('/master/department-master/export/pdf')->assertOk()));
        $this->assertStringStartsWith('PK', $this->bytes($this->fetch('/master/department-master/export/excel')->assertOk()));

        $narrowed = $this->bytes($this->fetch('/master/department-master/export/csv?cols=sno,status'));
        $this->assertSame(['S. No.', 'Status'], str_getcsv(str_getcsv($narrowed, "\n")[5]));

        $filtered = $this->bytes($this->fetch('/master/department-master/export/csv?q=zzzznotadept'));
        $this->assertStringContainsString('Search: zzzznotadept', $filtered);
    ...
    {
        $this->fetch('/master/department-master/export/exe')->assertNotFound();
    }
    ...
    {
        $user = $this->actor();
        $before = DepartmentMaster::count();

        DB::beginTransaction();

        try {
            $name = 'Gate Dept ' . substr(uniqid(), -6);

            $created = $this->actingAs($user)
                ->withHeaders(['X-Requested-With' => 'XMLHttpRequest', 'Accept' => 'application/json'])
                ->postJson('/master/department-master/store', ['pk' => '', 'department_name' => $name]);

            $created->assertOk()->assertJson(['status' => true]);

            $row = DepartmentMaster::where('department_name', $name)->first();
            ...
            $renamed = 'Gate Dept ' . substr(uniqid(), -6);
            $this->actingAs($user)
                ->withHeaders(['X-Requested-With' => 'XMLHttpRequest', 'Accept' => 'application/json'])
                ->postJson('/master/department-master/store', [
                    'pk' => encrypt($row->pk), 'department_name' => $renamed,
                ])
                ->assertOk()
                ->assertJson(['status' => true]);

            $this->assertSame($renamed, $row->fresh()->department_name);

            // varchar(100): a longer name must be rejected, not truncated by MySQL.
            $this->actingAs($user)
                ->withHeaders(['X-Requested-With' => 'XMLHttpRequest', 'Accept' => 'application/json'])
                ->postJson('/master/department-master/store', ['pk' => '', 'department_name' => str_repeat('x', 101)])
                ->assertStatus(422)
                ->assertJsonValidationErrors('department_name');
        ...
        $this->assertSame($before, DepartmentMaster::count(), 'the test left rows behind');
    }
}
