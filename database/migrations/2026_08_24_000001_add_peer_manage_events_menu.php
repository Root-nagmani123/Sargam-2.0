...
/**
 * Adds the "Manage Events" sidebar link under Peer Evaluation (Setup > FC Forms),
 * alongside the existing "Peer Evaluation Admin" child, plus its Spatie
 * permission so it can be granted to non-admin roles.
 *
 * The breadcrumb on that page is derived from this row (SidebarNavResolver
 * walks category -> group -> parent chain), so without it the page would fall
 * back to the legacy section-based trail and read "Home / Academic / Manage
 * Events" instead of "Home / Setup / FC Forms / Peer Evaluation / Manage Events".
 *
 * Idempotent.
 */
return new class extends Migration
{
    private string $route = 'admin/peer/events';
    private string $permissionName = 'peer_manage_events';
    private string $menuName = 'Manage Events';
