...
/**
 * Places "Define Club/ Society" in the sidebar under
 * Communications → Club/ Society, and creates its Spatie permission so the menu
 * can be granted to non-admin roles (admins bypass the permission check in
 * MenuService::buildMenus()). Idempotent.
 */
return new class extends Migration
{
    private string $route          = 'master/club-society';
    private string $permissionName = 'define_club_society';
    private string $menuName       = 'Define Club/ Society';
    ...
    public function down(): void
    {
        ...
        if ($category) {
            // Only drop the group if this migration's menu was the last one in it.
            $group = DB::table('menu_groups')
                ->where('category_id', $category->id)
                ->where('name', $this->groupName)
                ->first();
